...
int main(int argc, char* argv[])
{
  ...
  // 2. now we create a vector of x- and y-values for a simple plot (a sine
  // curve)
  //    in addition, a vector P will hold values that will be mapped onto
  //    different plot styles and a vector C that will be mapped onto different
  //    colors
  ...
  for (int i = 0; i < Ndata; i++) {
    ...
    S << 5 * (i + 1);
    LW << (i + 1) * 1.5;
    RGB << double(qRgb(double(i) / double(Ndata) * 255, 0,
                       255 - double(i) / double(Ndata) * 255));
  }
  ...
  // 3. now we make several graphs. In each one, another property of the graph
  // is controlled by
  //    a datacolumn

  // 3.1 for graph1, we use dataset X/Y1
  //     and the symbol type is choose according to the contents of column P:
  JKQTPXYParametrizedScatterGraph* graph1 =
    new JKQTPXYParametrizedScatterGraph(&plot);
  ...
  /*graph1->setSymbolColumnFunctor([](double x, double y, double sym) ->
  JKQTPGraphSymbols { if (sym<Ndata/2) { return JKQTPGraphSymbols::JKQTPCircle;
      } else if (sym>Ndata/2) {
  ...
  JKQTPXYParametrizedScatterGraph* graph2 =
    new JKQTPXYParametrizedScatterGraph(&plot);
  ...
  JKQTPXYParametrizedScatterGraph* graph3 =
    new JKQTPXYParametrizedScatterGraph(&plot);
  ...
  JKQTPXYParametrizedScatterGraph* graph4 =
    new JKQTPXYParametrizedScatterGraph(&plot);
  ...
  JKQTPXYParametrizedScatterGraph* graph5 =
    new JKQTPXYParametrizedScatterGraph(&plot);
  ...
  JKQTPXYParametrizedScatterGraph* graph6 =
    new JKQTPXYParametrizedScatterGraph(&plot);
  ...
  //     and the line width is set by column LW and the color from palette
  //     JKQTPMathImageBLUEYELLOW via column C:
  JKQTPXYParametrizedScatterGraph* graph7 =
    new JKQTPXYParametrizedScatterGraph(&plot);
  ...
  // 4. autoscale the plot so the graph is contained and format the coordinate
  // system and key
