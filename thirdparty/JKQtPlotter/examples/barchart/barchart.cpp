...
void doExample()
{
  ...
  QString L[Ndata] = { "cat. A", "cat. C", "cat. B", "cat. D",
                       "other" }; // unsorted category axis
  double X[Ndata] = { 1, 3, 2, 4, 5 };
  // QString L[Ndata]={  "cat. A", "cat. B", "cat. C", "cat. D", "other"}; //
  // correctly sorted data! double  X[Ndata]={        1,        2,        3, 4,
  // 5};
  ...
  //    of the newly created columns with names "x" and "y..." and the (copied)
  //    data from X and Y...
  ...
  if (dynamic_cast<JKQTPBarVerticalGraph*>(graph1) != nullptr) {
    ...
    //    labels for these groups (stored in L). In order to display these
    //    labels, we have to tell the x-Axis to use these special labels:
