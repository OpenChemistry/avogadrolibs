 ...
 * JKQTPlotter: Examples: Draw an Artistic Image with a Parametrized Scatter
 * Graph
 *
...
int main(int argc, char* argv[])
{
  ...
  for (int y = 0; y < image.height(); y++) {
    for (int x = 0; x < image.width(); x++) {
      ...
      // calculate point sizes from inverse grey value and scaling between 0 and
      // maxSymbolSize
      pointSizes.push_back(static_cast<double>(255 - qGray(image.pixel(x, y))) /
                           255.0 * maxSymbolSize);
  ...
  // 3.1 Now add a parametrized scatter graph with columnX, columnY for the
  // positions of the
  //     scatter points, where the symbol size is given by column columnS and
  //     the color of each symbol is set from column columnG, via a color
  ...
  // 4. scale the plot so the graph is contained and format the coordinate
  // system
  ...
  // ensure that axis aspect ratio and coordinate system aspect ratio are
  // maintained
