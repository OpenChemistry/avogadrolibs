...
int main(int argc, char* argv[])
{
  ...
  //    are shown
  QVector<JKQTPErrorPlotstyle> errorStyles{ JKQTPNoError,
                                            JKQTPErrorBars,
  ...
  QVector<bool> showXandYError{ false, true,  true,  false, false, true,
  ...
  for (int errorID = 0; errorID < errorStyles.size(); errorID++) {
    ...
    // graph title is made from symbol+errorStylestyle, we use the LaTeX
    // instruction \verb around the result of JKQTPErrorPlotstyle2String(),
    // because it contains underscores that would otherwise lead to lower-case
    // letter, which we don't want
    graph->setTitle("\\verb{" +
                    JKQTPErrorPlotstyle2String(errorStyles[errorID]) + "}");
