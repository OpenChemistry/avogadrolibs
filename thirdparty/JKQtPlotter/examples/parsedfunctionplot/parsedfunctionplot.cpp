...
int main(int argc, char* argv[])
{
  ...
  edit->setToolTip("enter a function in dependence of the variable <tt>x</tt> "
                   "and press ENTER to update the graph");
  ...
  spinP1->setToolTip("enter a Value for parameter <tt>p1</tt> and press ENTER "
                     "to update the graph");
  QDoubleSpinBox* spinP2 = new QDoubleSpinBox(&mainWin);
  spinP2->setValue(4);
  spinP2->setRange(-10000, 10000);
  spinP2->setToolTip("enter a Value for parameter <tt>p1</tt> and press ENTER "
                     "to update the graph");
  ...
  // 2. now we add a JKQTPXParsedFunctionLineGraph object, which will draw the
  // function from
  //    the line edit
  JKQTPXParsedFunctionLineGraph* parsedFunc =
    new JKQTPXParsedFunctionLineGraph(plot);
  plot->addGraph(parsedFunc);
  //    finally we connect the line edit with the graph, whenever RETURN is
  //    pressed, the graph is updated:
  auto updateGraphFunctor = [=]() {
    parsedFunc->setTitle(
      QString("user function: \\verb{" + edit->text() + "}, p_1=%1, p_2=%2")
        .arg(spinP1->value())
        .arg(spinP2->value()));
  ...
  QObject::connect(spinP1, &QDoubleSpinBox::editingFinished,
                   updateGraphFunctor);
  QObject::connect(spinP2, &QDoubleSpinBox::editingFinished,
                   updateGraphFunctor);
