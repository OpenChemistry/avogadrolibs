...
TestWidgetRGBImages::TestWidgetRGBImages(QWidget* parent) : QWidget(parent)
{
  // plotter for images
  ...
  plotImgRGB->getPlotter()->setUserSettigsFilename(
    QApplication::applicationDirPath() + "/usersettings.ini", "plot_img_rgb/");
  ...
  connect(chkColorbars, SIGNAL(toggled(bool)), this,
          SLOT(setRGBColorBars(bool)));
  ...
  for (int x = 0; x < 100; x++) {
    for (int y = 0; y < 100; y++) {
      rgb_rimg2[y * 100 + x] =
        sin(2 * JKQTPSTATISTICS_PI * sqrt(1.0 + x * x + y * y) / 20);
      rgb_bimg2[y * 100 + x] =
        sin(2 * JKQTPSTATISTICS_PI * sqrt(1.0 + x * x + y * y) / 30);
      rgb_gimg2[y * 100 + x] =
        cos(2 * JKQTPSTATISTICS_PI * sqrt(1.0 + x * x + y * y) / 30);
      rgb_aimg2[y * 100 + x] = double(x) / 100.0;
    }
  }
  rgbimg =
    new JKQTPRGBMathImage(0, 0, 10, 10, JKQTPMathImageDataType::DoubleArray,
                          rgb_rimg2, 100, 100, plotImgRGB->getPlotter());
  ...
  rgbimg->setData(rgb_rimg2, rgb_gimg2, rgb_bimg2, 100, 100,
  ...
  rgbimg2 = new JKQTPColumnRGBMathImage(10, 0, 10, 10, 100, 100,
                                        plotImgRGB->getPlotter());
  ...
  rgbimg2->setNx(100);
  rgbimg2->setNy(100);
  rgbimg2->setImageRColumn(
    rgb_colr =
      plotImgRGB->getDatastore()->addCopiedColumn(rgb_bimg2, 100 * 100, "R"));
  rgbimg2->setImageGColumn(
    rgb_colg =
      plotImgRGB->getDatastore()->addCopiedColumn(rgb_gimg2, 100 * 100, "G"));
  rgbimg2->setImageBColumn(
    rgb_colb =
      plotImgRGB->getDatastore()->addCopiedColumn(rgb_rimg2, 100 * 100, "B"));
  rgbimg2->setModifierColumn(
    rgb_cola = plotImgRGB->getDatastore()->addCopiedColumn(rgb_aimg2, 100 * 100,
                                                           "alpha"));
  rgbimg2->setModifierMode(JKQTPMathImageModifierMode::ModifyNone);
  // rgbimg2->setData(rgb_rimg2, rgb_gimg2, rgb_bimg2, 100, 100,
  // JKQTPMathImageDataType::DoubleArray);
