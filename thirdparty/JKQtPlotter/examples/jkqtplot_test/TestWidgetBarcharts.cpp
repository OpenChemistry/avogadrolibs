...
TestWidgetBarcharts::TestWidgetBarcharts(QWidget* parent) : QWidget(parent)
{
  ...
  plotBarchart->getPlotter()->setUserSettigsFilename(
    QApplication::applicationDirPath() + "/usersettings.ini", "plot_peaks/");
  ...
  size_t bcpxd =
    plotBarchart->getDatastore()->addCopiedColumn(dataBCXD, "x_date");
  size_t bcpy1 = plotBarchart->getDatastore()->addCopiedColumn(dataBCY1, "y1");
  size_t bcpye1 =
    plotBarchart->getDatastore()->addCopiedColumn(dataBCYE1, "ye1");
  size_t bcpy2 = plotBarchart->getDatastore()->addCopiedColumn(dataBCY2, "y2");
  size_t bcpye2 =
    plotBarchart->getDatastore()->addCopiedColumn(dataBCYE2, "ye2");

  JKQTPBarVerticalErrorGraph* plteBar1 =
    new JKQTPBarVerticalErrorGraph(plotBarchart->getPlotter());
  ...
  JKQTPBarVerticalErrorGraph* plteBar2 =
    new JKQTPBarVerticalErrorGraph(plotBarchart->getPlotter());
  ...
  plotBarchart2->getPlotter()->setUserSettigsFilename(
    QApplication::applicationDirPath() + "/usersettings.ini", "plot_peaks/");

  barchartLayout->addWidget(plotBarchart2);

  JKQTPBarHorizontalErrorGraph* plteBar3 =
    new JKQTPBarHorizontalErrorGraph(plotBarchart2->getPlotter());
  ...
  JKQTPBarHorizontalErrorGraph* plteBar4 =
    new JKQTPBarHorizontalErrorGraph(plotBarchart2->getPlotter());
  ...
  connect(chkBarLog, SIGNAL(toggled(bool)), this,
          SLOT(setBarchartLogLog(bool)));
  ...
  connect(spinBarLabelAngel, SIGNAL(valueChanged(int)), this,
          SLOT(setBarchartAngele(int)));
  ...
  connect(spinBarLabelAngel2, SIGNAL(valueChanged(int)), this,
          SLOT(setBarchartAngele2(int)));
}
