...
TestWidgetGraphs::TestWidgetGraphs(QWidget* parent) : QWidget(parent)
{
  ...
  plot->getPlotter()->setUserSettigsFilename(
    QApplication::applicationDirPath() + "/usersettings.ini", "plot/");
  plotBot->getPlotter()->setUserSettigsFilename(
    QApplication::applicationDirPath() + "/usersettings.ini", "plot/");
  plotBot2->getPlotter()->setUserSettigsFilename(
    QApplication::applicationDirPath() + "/usersettings.ini", "plot/");
  ...
  plot->getPlotter()->getXAxis()->setAxisLabel(
    "$x$ axis with greek letters $\\sqrt{\\alpha\\cdot\\beta}$");
  plot->getPlotter()->getYAxis()->setAxisLabel(
    "$y$-axis label $f(x)=\\sin\\left(x^2\\right)$ [unit]");

  plotBot->getPlotter()->getXAxis()->setAxisLabel(
    plot->getPlotter()->getXAxis()->getAxisLabel());
  plotBot->getPlotter()->getYAxis()->setAxisLabel("$y$-axis 2 [unit]");
  plotBot2->getPlotter()->getXAxis()->setAxisLabel(
    plot->getPlotter()->getXAxis()->getAxisLabel());
  ...
  // plot->addGraphWithXError(cy12, cx1, cy13, "$\\cos(x)$ with errors",
  // JKQTPFilledCurveY, JKQTPErrorPolygons);
  ...
  size_t id =
    plot->getPlotter()->addGraph(fcxgr = new JKQTPFilledCurveXGraph(plot));
  ...
  size_t yeb = plot->getPlotter()->addGraph(
    gELines = new JKQTPFilledCurveXErrorGraph(plot));
  ...
  for (unsigned int i = 0; i < N3; i++) {
    ...
    // plot->getXAxis()->addAxisTickLabel(i+1, QString("$\\pi_{%1}\\cdot
    // 10^{%2}$").arg(i+1).arg(i+1-N3)); plot->getYAxis()->addAxisTickLabel(i+1,
    // QString("$\\pi_{%1}\\cdot 10^{%2}$").arg(i+1).arg(i+1-N3));
  }
  ...
  JKQTPBoxplotVerticalGraph* box =
    new JKQTPBoxplotVerticalGraph(plotBot2->getPlotter());
  ...
  connect(chklogX, SIGNAL(toggled(bool)), plot->getPlotter()->getXAxis(),
          SLOT(setLogAxis(bool)));
  connect(chklogY, SIGNAL(toggled(bool)), plot->getPlotter()->getYAxis(),
          SLOT(setLogAxis(bool)));
  connect(chklogX, SIGNAL(toggled(bool)), plotBot2->getPlotter()->getXAxis(),
          SLOT(setLogAxis(bool)));
  connect(chklogY, SIGNAL(toggled(bool)), plotBot2->getPlotter()->getYAxis(),
          SLOT(setLogAxis(bool)));
  ...
  connect(chkInvertX, SIGNAL(toggled(bool)), plot->getPlotter()->getXAxis(),
          SLOT(setInverted(bool)));
  connect(chkInvertY, SIGNAL(toggled(bool)), plot->getPlotter()->getYAxis(),
          SLOT(setInverted(bool)));
  ...
  connect(cmbJKQTPPlotSymbol, SIGNAL(currentIndexChanged(int)), this,
          SLOT(setESSymbol(int)));
  layout->addWidget(cmbJKQTPPlotSymbol);
  JKQTPErrorPlotstyleComboBox* cmbErrors =
    new JKQTPErrorPlotstyleComboBox(this);
  cmbErrors->setCurrentErrorStyle(JKQTPErrorBars);
  connect(cmbErrors, SIGNAL(currentIndexChanged(int)), this,
          SLOT(setErrorLineStyle(int)));
  ...
  connect(spinScale, SIGNAL(valueChanged(double)), plot,
          SLOT(setMagnification(double)));
  connect(spinScale, SIGNAL(valueChanged(double)), plotBot,
          SLOT(setMagnification(double)));
  connect(spinScale, SIGNAL(valueChanged(double)), plotBot2,
          SLOT(setMagnification(double)));
  ...
  connect(cmbSortOrder2, SIGNAL(currentIndexChanged(int)), this,
          SLOT(setSortOrder2(int)));
  layout->addWidget(cmbSortOrder2);

  JKQTPKeyLayoutComboBox* cmbKeyLayout = new JKQTPKeyLayoutComboBox(this);
  connect(cmbKeyLayout, SIGNAL(currentLayoutChanged(JKQTPKeyLayout)), this,
          SLOT(setKeyLayout2(JKQTPKeyLayout)));
  layout->addWidget(cmbKeyLayout);

  JKQTPKeyPositionComboBox* cmbKeyPos = new JKQTPKeyPositionComboBox(this);
  connect(cmbKeyPos, SIGNAL(currentPositionChanged(JKQTPKeyPosition)), this,
          SLOT(setKeyPos2(JKQTPKeyPosition)));
...
void TestWidgetGraphs::setErrorLineStyle(int /*index*/)
{
  ...
  if (cmb) {
    ...
    //        JKQTPXGraphErrors*
    //        ex=dynamic_cast<JKQTPXGraphErrors*>(plteErrors);
    JKQTPYGraphErrors* ey = dynamic_cast<JKQTPYGraphErrors*>(plteErrors);
    if (exy) {
      exy->setYErrorStyle(String2JKQTPErrorPlotstyle(
        cmb->itemData(cmb->currentIndex()).toString()));
    }
    if (ey) {
      ey->setYErrorStyle(String2JKQTPErrorPlotstyle(
        cmb->itemData(cmb->currentIndex()).toString()));
    }
    plot->redrawPlot();
  }
...
void TestWidgetGraphs::setESSymbol(int /*index*/)
{
  ...
  if (cmb) {
    ...
    if (exy) {
      exy->setSymbolType(String2JKQTPGraphSymbols(
        cmb->itemData(cmb->currentIndex()).toString()));
    }
...
{
  if (index == 0)
    pltePlot2->setDataSortOrder(JKQTPXYGraph::Unsorted);
  if (index == 1)
    pltePlot2->setDataSortOrder(JKQTPXYLineGraph::SortedX);
  if (index == 2)
    pltePlot2->setDataSortOrder(JKQTPXYLineGraph::SortedY);
  plotBot->redrawPlot();
}
