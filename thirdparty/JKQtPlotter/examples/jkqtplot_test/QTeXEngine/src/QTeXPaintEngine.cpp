...
QTeXPaintEngine::QTeXPaintEngine(const QString& f, QTeXPaintDevice::Unit u)
  : QPaintEngine(QPaintEngine::AllFeatures), fname(f), d_pixmap_index(1),
    d_unit(u)
{
  d_pgf_mode = false;
  d_open_scope = false;
...
bool QTeXPaintEngine::begin(QPaintDevice* p)
{
  ...
  if (file->open(QIODevice::WriteOnly)) {
    ...
    if (d_document_mode) {
      ...
      t << "\\usepackage[left=0.2cm,top=0.1cm,right=0.2cm,nohead,nofoot]{"
           "geometry}\n";
...
void QTeXPaintEngine::drawPoints(const QPointF* points, int pointCount)
{
  ...
  for (int i = 0; i < pointCount; i++) {
    ...
    if (d_pgf_mode) {
      ...
      path += " rectangle " +
              tikzPoint(convertPoint(QPointF(p.x() + lw, p.y() + lw))) + ";\n";
      s += "\\fill " + path;
    }
  }
...
void QTeXPaintEngine::drawPolygon(const QPointF* points, int pointCount,
...
void QTeXPaintEngine::drawTextItem(const QPointF& p, const QTextItem& textItem)
{
  ...
  if (d_font_size)
    s += "\\fontsize{" + QString::number(int(f.pointSizeF())) +
         "}{0}\\selectfont{";
...
void QTeXPaintEngine::drawEllipse(const QRectF& rect)
{
  ...
  QString path;
  if (d_pgf_mode) {
    path = pgfPoint(convertPoint(
      QPointF(p.x() + 0.5 * rect.width(), p.y() - 0.5 * rect.height())));
    path += pgfPoint(QPointF(0, 0.5 * rect.height() * resFactorY()));
    path += pgfPoint(QPointF(0.5 * rect.width() * resFactorX(), 0)) + "\n";
  } else {
    path = tikzPoint(convertPoint(
      QPointF(p.x() + 0.5 * rect.width(), p.y() - 0.5 * rect.height())));
    path += " ellipse (";
    QString u = unit();
...
void QTeXPaintEngine::drawImage(const QRectF& r, const QImage& image,
                                const QRectF& sr,
...
void QTeXPaintEngine::drawPixmap(const QRectF& r, const QPixmap& pm,
                                 const QRectF& sr)
{
...
QString QTeXPaintEngine::tikzBrush(const QBrush& brush)
{
  ...
  switch (brush.style()) {
    ...
    case Qt::LinearGradientPattern: {
      const QLinearGradient* qtgradient =
    ...
    case Qt::RadialGradientPattern: {
      const QRadialGradient* qtgradient =
      ...
      qWarning("QTeXEngine: Uncentered Qt::RadialGradientPattern with more "
               "than two colors not supported.");
...
{
  return QPointF(resFactorX() * p.x(),
                 paintDevice()->height() - resFactorY() * p.y());
}
...
QString QTeXPaintEngine::pgfPen(const QPen& pen)
{
  ...
  s += color(pen.color());
  s +=
    "\\pgfsetlinewidth{" + QString::number(painter()->pen().widthF()) + "pt}\n";
  ...
  switch (pen.joinStyle()) {
    case Qt::MiterJoin:
      s += "\\pgfsetmiterjoin\n";
      // s += "\\pgfsetmiterlimit{" + QString::number(pen.miterLimit()) +
      // "pt}\n";
      break;
    case Qt::BevelJoin:
      s += "\\pgfsetbeveljoin\n";
