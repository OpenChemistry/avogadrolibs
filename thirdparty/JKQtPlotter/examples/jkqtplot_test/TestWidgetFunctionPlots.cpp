  ...
  : QWidget(parent)
{
  ...
  connect(pfuncErrorStyle, SIGNAL(currentIndexChanged(int)), this,
          SLOT(setPFuncStyle()));
  chkPFuncDrawSamples = new QCheckBox(tr("display samples"), this);
  plotFuncLayout->addWidget(chkPFuncDrawSamples);
  connect(chkPFuncDrawSamples, SIGNAL(toggled(bool)), this,
          SLOT(setPFuncStyle()));
  ...
  connect(spinPFunc, SIGNAL(valueChanged(int)), this,
          SLOT(setPFuncMinPoint(int)));
  ...
  connect(edtPFunc, SIGNAL(currentIndexChanged(QString)), this,
          SLOT(setPFunc(QString)));
  ...
  plotFuncLayout->addWidget(edtPFunc);
  connect(edtPFunc, SIGNAL(currentIndexChanged(QString)), this,
          SLOT(setPFuncE(QString)));
  QCheckBox* chkFLogX = new QCheckBox(tr("x-axis: log-scale"));
  chkFLogX->setChecked(false);
  connect(chkFLogX, SIGNAL(toggled(bool)), plotFuncPlt->getXAxis(),
  ...
  connect(chkFLogY, SIGNAL(toggled(bool)), plotFuncPlt->getYAxis(),
          SLOT(setLogAxis(bool)));
...
void TestWidgetFunctionPlots::setPFuncStyle()
{
  ...
  if (pfuncErrorStyle->getErrorLineStyle() == JKQTPErrorLines)
    pfunc->setDrawErrorLines(true);
  if (pfuncErrorStyle->getErrorLineStyle() == JKQTPErrorPolygons)
    pfunc->setDrawErrorPolygons(true);
