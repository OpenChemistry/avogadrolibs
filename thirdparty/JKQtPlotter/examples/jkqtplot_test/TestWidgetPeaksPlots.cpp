...
TestWidgetPeaksPlots::TestWidgetPeaksPlots(QWidget* parent) : QWidget(parent)
{
  ...
  plotPeaks->getPlotter()->setUserSettigsFilename(
  ...
  connect(chkDrawBaseline, SIGNAL(toggled(bool)), this,
          SLOT(setDrawBaseline(bool)));
  ...
  for (int i = 0; i < 1000; i++) {
    ...
    const double p = 0.33 * fabs(sin(x * 4.0 * JKQTPSTATISTICS_PI)) *
                     fabs(sin(x * 4.0 * JKQTPSTATISTICS_PI));
