  ...
  : QWidget(parent)
{
  ...
  plotContour->getPlotter()->setUserSettigsFilename(
    QApplication::applicationDirPath() + "/usersettings.ini", "contour/");
  layoutContour->addWidget(plotContour);

  JKQTPlotter* plotDensity = new JKQTPlotter(true, this);
  plotContour->setPlotUpdateEnabled(false);
  plotContour->getPlotter()->setPlotLabel(
    tr("\\textbf{2D histogram plot with contours}"));
  plotDensity->getPlotter()->setPlotLabel(tr("\\textbf{2D histogram}"));
  plotContour->setObjectName("density");
  plotContour->getPlotter()->setUserSettigsFilename(
    QApplication::applicationDirPath() + "/usersettings.ini", "density/");
  ...
  size_t colHist2D = ds->addCopiedImageAsColumn(histAlex.data(), histAlexNx,
                                                histAlexNy, tr("2Dhist"));
  //    size_t
  //    colContour=plotContour->getDatastore()->addCopiedImageAsColumnTranspose(histAlex.data(),
  //    histAlexNx, histAlexNy, tr("2Dhist"));

  JKQTPColumnMathImage* densityplot =
    new JKQTPColumnMathImage(plotDensity->getPlotter());
  ...
  levels << 4 << 5 << 9 << 14;
  JKQTPColumnContourPlot* cp =
    new JKQTPColumnContourPlot(plotContour->getPlotter());
  cp->setX(-0.1);
  cp->setY(-0.1);
  cp->setWidth(1.1);
  cp->setHeight(1.1);
