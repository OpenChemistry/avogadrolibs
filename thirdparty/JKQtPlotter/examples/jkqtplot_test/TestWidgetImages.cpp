...
TestWidgetImages::TestWidgetImages(QWidget* parent) : QWidget(parent)
{
  ...
  plotImg->getPlotter()->setUserSettigsFilename(
    QApplication::applicationDirPath() + "/usersettings.ini", "plot_img/");
  ...
  JKQTPImage* pimg1 =
    new JKQTPImage(0, 0, 10, 20, &img1, plotImg->getPlotter());
  pimg1->setTitle("LENA");
  ...
  for (int x = 0; x < 100; x++) {
    for (int y = 0; y < 100; y++) {
      ...
      bimg2[y * 100 + x] =
        static_cast<double>(rand()) / static_cast<double>(RAND_MAX) > 0.9;
    }
  }
  pimg2 =
    new JKQTPMathImage(10, 0, 10, 10, JKQTPMathImageDataType::DoubleArray, img2,
                       100, 100, JKQTPMathImageGRAY, plotImg->getPlotter());
  pimg2->setImageMin(-1);
  ...
  pimg3 =
    new JKQTPMathImage(20, 0, 10, 10, JKQTPMathImageDataType::DoubleArray, img2,
                       100, 100, JKQTPMathImageMATLAB, plotImg->getPlotter());
  ...
  pimg3->setImageName(
    "I(x,y)=\\sin\\left(\\frac{2\\pi\\cdot\\sqrt{1+x^2+y^2}}{20}\\right)");
  ...
  ty.setAlphaF(0.1);
  JKQTPOverlayImage* oimg1 = new JKQTPOverlayImage(
    10, 0, 10, 10, bimg2, 100, 100, QColor("blue"), plotImg->getPlotter());
  oimg1->setTitle("overlay");
  oimg1->setFalseColor(ty);
  plotImg->getPlotter()->addGraph(oimg1);

  JKQTPOverlayImageEnhanced* oimg2 = new JKQTPOverlayImageEnhanced(
    10, 0, 10, 10, bimg2, 100, 100, QColor("red"), plotImg->getPlotter());
  ...
  JKQTPMathImageColorPaletteComboBox* cmbPal =
    new JKQTPMathImageColorPaletteComboBox(this);
  layWid->addRow("left image palette", cmbPal);
  connect(cmbPal, SIGNAL(currentIndexChanged(int)), this,
          SLOT(setImgPalette1(int)));
  cmbPal = new JKQTPMathImageColorPaletteComboBox(this);
  layWid->addRow("right image palette", cmbPal);
  connect(cmbPal, SIGNAL(currentIndexChanged(int)), this,
          SLOT(setImgPalette(int)));
