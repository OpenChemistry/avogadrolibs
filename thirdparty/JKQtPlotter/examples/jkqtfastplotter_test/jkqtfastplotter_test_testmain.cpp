...
TestMain::TestMain(QWidget* parent) : QWidget(parent)
{
  ...
  JKQTFPLinePlot* pv =
    new JKQTFPLinePlot(pl1, &xx, &yy, QColor("black"), Qt::SolidLine, 3);
  ...
  for (int x = 0; x < IMAGE_N; x++) {
    for (int y = 0; y < IMAGE_N; y++) {
      image[y * IMAGE_N + x] =
        IMAGE_N * (1.1 + sin(sqrt((x - 50) * (x - 50) + (y - 50) * (y - 50))));
      if (x > 50 && y > 50)
        image[y * IMAGE_N + x] = 0;
      if (x < 50 && y < 50)
        image[y * IMAGE_N + x] = 50;
      if (abs(x - (IMAGE_N - y)) < 4)
        ovrl[y * IMAGE_N + x] = true;
      else
        ovrl[y * IMAGE_N + x] = false;
    }
  }

  JKQTFPimagePlot* p5 = new JKQTFPimagePlot(pl3, image, JKQTFP_double, IMAGE_N,
                                            IMAGE_N, 0, 10, 0, 10, JKQTFP_GRAY);
  QColor col = QColor("red");
  col.setAlpha(127);
  JKQTFPimageOverlayPlot* p5o =
    new JKQTFPimageOverlayPlot(pl3, ovrl, IMAGE_N, IMAGE_N, 0, 10, 0, 10, col);
  ...
  for (int x = 0; x < IMAGE_N; x++) {
    for (int y = 0; y < IMAGE_N; y++) {
      imageRed[y * IMAGE_N + x] =
        static_cast<double>(std::abs(x) + std::abs(y)) / 2.0;
      imageGreen[y * IMAGE_N + x] =
        (fabs(double(IMAGE_N) - x) + static_cast<double>(std::abs(y))) / 2.0;
      imageBlue[y * IMAGE_N + x] = sqrt(x * x + y * y);
    }
  }

  prgb = new JKQTFPRGBImageOverlayPlot(p21);
  prgb->setImage(imageRed, JKQTFP_double, imageGreen, JKQTFP_double, imageBlue,
                 JKQTFP_double, IMAGE_N, IMAGE_N, 0, 10, 0, 10);
  p21->addPlot(prgb);
