...
TestUserInteraction::TestUserInteraction(QWidget* parent) : QMainWindow(parent)
{
  ...
  // Some of the are also available in the context menu and toolbar of the
  // JKQTPlotter others are not
  ...
  menuPlotS->addAction(
    plot->getPlotter()
      ->getActionSavePDF()); // not available from JKQTPlotter by default
  ...
  connect(chkPositionDisplay, SIGNAL(toggled(bool)), plot,
          SLOT(setMousePositionShown(bool)));
  ...
  // add a checkbox to switch the toolbar between always visible and the hiding
  // mode, where it only appears, when the mouse hovers over its designated area
  chkToolbarAlwaysOn = new QCheckBox(tr("toolbar 'always on'"), this);
  chkToolbarAlwaysOn->setChecked(plot->isToolbarAlwaysOn());
  connect(chkToolbarAlwaysOn, SIGNAL(toggled(bool)), plot,
          SLOT(setToolbarAlwaysOn(bool)));
  ...
  connect(cmbMagnification, SIGNAL(currentIndexChanged(int)), this,
          SLOT(setPlotMagnification(int)));
  cmbMagnification->setCurrentIndex(3);

  // add a QComboBox that allows to set the left mouse button action for the
  // JKQTPlotter
  cmbLeftNoModMouseAction = new QComboBox(this);
  layForm->addRow("mouse action: left-click, no modifiers",
  ...
  connect(cmbLeftNoModMouseAction, SIGNAL(currentIndexChanged(int)), this,
          SLOT(setLeftMouseAction(int)));
  setLeftMouseAction(cmbLeftNoModMouseAction->currentIndex());

  // add a QComboBox that allows to set the left mouse button action for the
  // JKQTPlotter
  cmbLeftCtrlModMouseAction = new QComboBox(this);
  layForm->addRow("mouse action: left-click, Ctrl modifier",
  ...
  connect(cmbLeftCtrlModMouseAction, SIGNAL(currentIndexChanged(int)), this,
          SLOT(setLeftCtrlMouseAction(int)));
  setLeftCtrlMouseAction(cmbLeftCtrlModMouseAction->currentIndex());

  // add a QComboBox that allows to set the left mouse button action for the
  // JKQTPlotter
  cmbRightNoModMouseAction = new QComboBox(this);
  layForm->addRow("mouse action: right-click, no modifiers",
  ...
  connect(cmbRightNoModMouseAction, SIGNAL(currentIndexChanged(int)), this,
          SLOT(setRightMouseAction(int)));
  setRightMouseAction(cmbRightNoModMouseAction->currentIndex());

  // add a QComboBox that allows to set whether the right mouse button may show
  // the context menu on a single click
  ...
  connect(cmbRightClickContextMenu, SIGNAL(currentIndexChanged(int)), this,
          SLOT(setContextMenuMode(int)));
  ...
  special->addMenu("Special submenu")
    ->addAction("Special subentry 1 (no action!)");
  plot->setSpecialContextMenu(special);

  // add a QComboBox that allows to set the left mouse button double-click
  // action for the JKQTPlotter
  cmbLeftDoubleClickMouseAction = new QComboBox(this);
  layForm->addRow("mouse action: left double-click, no modifiers",
  ...
  cmbLeftDoubleClickMouseAction->addItem(
  ...
  connect(cmbLeftDoubleClickMouseAction, SIGNAL(currentIndexChanged(int)), this,
          SLOT(setLeftDoubleClickMouseAction(int)));
  ...
  layForm->addRow("mouse action: right double-click, no modifiers",
  ...
  cmbRightDoubleClickMouseAction->addItem(
    "jkqtpdcaClickOpensSpecialContextMenu");
  ...
  connect(cmbRightDoubleClickMouseAction, SIGNAL(currentIndexChanged(int)),
          this, SLOT(setRightDoubleClickMouseAction(int)));
  setRightDoubleClickMouseAction(
    cmbRightDoubleClickMouseAction->currentIndex());

  // add a QComboBox that allows to set the mouse wheel action without modifiers
  cmbMouseWheelAction = new QComboBox(this);
  layForm->addRow("mouse action: mouse wheel, no modifiers",
                  cmbMouseWheelAction);
  ...
  connect(cmbMouseWheelAction, SIGNAL(currentIndexChanged(int)), this,
          SLOT(setMouseWheelNoModAction(int)));
  ...
  connect(cmbUserActionMarkerType, SIGNAL(currentIndexChanged(int)), this,
          SLOT(setUserActionMarkerType(int)));
  ...
  cmbUserActionMarkerColor->addItem(
    jkqtp_QColor2String(plot->getUserActionMarkerPen().color()));
  ...
  connect(cmbUserActionMarkerColor, SIGNAL(currentIndexChanged(int)), this,
          SLOT(setUserActionMarkerColor(int)));
  ...
  layForm2->addRow("mouse action: opaque outline color",
                   cmbUserActionOpaqueColor);
  cmbUserActionOpaqueColor->addItem(
    jkqtp_QColor2String(plot->getUserActionOpaquePen().color()));
  ...
  connect(cmbUserActionOpaqueColor, SIGNAL(currentIndexChanged(int)), this,
          SLOT(setUserActionOpaqueColor(int)));
  ...
  layForm2->addRow("mouse action: opaque fill color",
                   cmbUserActionOpaqueFillColor);
  cmbUserActionOpaqueFillColor->addItem(
    jkqtp_QColor2String(plot->getUserActionOpaqueBrush().color()));
  ...
  connect(cmbUserActionOpaqueFillColor, SIGNAL(currentIndexChanged(int)), this,
          SLOT(setUserActionOpaqueFillColor(int)));
  setUserActionOpaqueFillColor(cmbUserActionOpaqueFillColor->currentIndex());

  // add a QComboBox that allows to set the outline color of user-action
  // Overlays
  cmbUserActionOverlayColor = new QComboBox(this);
  layForm2->addRow("mouse action: overlay outline color",
                   cmbUserActionOverlayColor);
  cmbUserActionOverlayColor->addItem(
    jkqtp_QColor2String(plot->getUserActionOverlayPen().color()));
  ...
  connect(cmbUserActionOverlayColor, SIGNAL(currentIndexChanged(int)), this,
          SLOT(setUserActionOverlayColor(int)));
  ...
  layForm2->addRow("mouse action: overlay fill color",
                   cmbUserActionOverlayFillColor);
  cmbUserActionOverlayFillColor->addItem(
    jkqtp_QColor2String(plot->getUserActionOverlayBrush().color()));
  ...
  connect(cmbUserActionOverlayFillColor, SIGNAL(currentIndexChanged(int)), this,
          SLOT(setUserActionOverlayFillColor(int)));
  ...
  connect(plot, SIGNAL(plotMouseMove(double, double)), this,
          SLOT(plotMouseMove(double, double)));
  connect(plot,
          SIGNAL(plotMouseClicked(double, double, Qt::KeyboardModifiers,
                                  Qt::MouseButton)),
          this,
          SLOT(plotMouseClicked(double, double, Qt::KeyboardModifiers,
                                Qt::MouseButton)));
  connect(plot,
          SIGNAL(plotMouseDoubleClicked(double, double, Qt::KeyboardModifiers,
                                        Qt::MouseButton)),
          this,
          SLOT(plotMouseDoubleClicked(double, double, Qt::KeyboardModifiers,
                                      Qt::MouseButton)));
  connect(plot,
          SIGNAL(plotMouseWheelOperated(double, double, Qt::KeyboardModifiers,
                                        int, int)),
          this,
          SLOT(plotMouseWheelOperated(double, double, Qt::KeyboardModifiers,
                                      int, int)));
  connect(plot,
          SIGNAL(plotNewZoomRectangle(double, double, double, double,
                                      Qt::KeyboardModifiers)),
          this,
          SLOT(plotNewZoomRectangle(double, double, double, double,
                                    Qt::KeyboardModifiers)));
  connect(plot, SIGNAL(contextMenuOpened(double, double, QMenu*)), this,
          SLOT(contextMenuOpened(double, double, QMenu*)));
  connect(
    plot,
    SIGNAL(zoomChangedLocally(double, double, double, double, JKQTPlotter*)),
    this,
    SLOT(zoomChangedLocally(double, double, double, double, JKQTPlotter*)));
  connect(
    plot,
    SIGNAL(
      userScribbleClick(double, double, Qt::KeyboardModifiers, bool, bool)),
    this,
    SLOT(userScribbleClick(double, double, Qt::KeyboardModifiers, bool, bool)));
  connect(plot,
          SIGNAL(userRectangleFinished(double, double, double, double,
                                       Qt::KeyboardModifiers)),
          this,
          SLOT(userRectangleFinished(double, double, double, double,
                                     Qt::KeyboardModifiers)));
  connect(plot,
          SIGNAL(userLineFinished(double, double, double, double,
                                  Qt::KeyboardModifiers)),
          this,
          SLOT(userLineFinished(double, double, double, double,
                                Qt::KeyboardModifiers)));
  connect(
    plot,
    SIGNAL(userCircleFinished(double, double, double, Qt::KeyboardModifiers)),
    this,
    SLOT(userCircleFinished(double, double, double, Qt::KeyboardModifiers)));
  connect(plot,
          SIGNAL(userEllipseFinished(double, double, double, double,
                                     Qt::KeyboardModifiers)),
          this,
          SLOT(userEllipseFinished(double, double, double, double,
                                   Qt::KeyboardModifiers)));
  connect(plot,
          SIGNAL(rulerDisplayed(double, double, double, double,
                                Qt::KeyboardModifiers)),
          this,
          SLOT(rulerDisplayed(double, double, double, double,
                              Qt::KeyboardModifiers)));
  connect(plot,
          SIGNAL(tooltipDisplayed(double, double, const QStringList&,
                                  const QList<JKQTPPlotElement*>&)),
          this,
          SLOT(tooltipDisplayed(double, double, const QStringList&,
                                const QList<JKQTPPlotElement*>&)));
...
{
  if (index == cmbLeftNoModMouseAction->count() - 1)
    plot->deregisterMouseDragAction(Qt::LeftButton, Qt::NoModifier);
  else
    plot->registerMouseDragAction(Qt::LeftButton, Qt::NoModifier,
                                  static_cast<JKQTPMouseDragActions>(index));
}

void TestUserInteraction::setLeftCtrlMouseAction(int index)
{
  if (index == cmbLeftCtrlModMouseAction->count() - 1)
    plot->deregisterMouseDragAction(Qt::LeftButton, Qt::ControlModifier);
  else
    plot->registerMouseDragAction(Qt::LeftButton, Qt::ControlModifier,
                                  static_cast<JKQTPMouseDragActions>(index));
}

void TestUserInteraction::setRightMouseAction(int index)
{
  if (index == cmbRightNoModMouseAction->count() - 1)
    plot->deregisterMouseDragAction(Qt::RightButton, Qt::NoModifier);
  else
    plot->registerMouseDragAction(Qt::RightButton, Qt::NoModifier,
                                  static_cast<JKQTPMouseDragActions>(index));
}
...
{
  if (index >= cmbLeftDoubleClickMouseAction->count() - 1)
    plot->deregisterMouseDoubleClickAction(Qt::LeftButton, Qt::NoModifier);
  else
    plot->registerMouseDoubleClickAction(
      Qt::LeftButton, Qt::NoModifier,
      static_cast<JKQTPMouseDoubleClickActions>(index));
}

void TestUserInteraction::setRightDoubleClickMouseAction(int index)
{
  if (index >= cmbLeftDoubleClickMouseAction->count() - 1)
    plot->deregisterMouseDoubleClickAction(Qt::RightButton, Qt::NoModifier);
  else
    plot->registerMouseDoubleClickAction(
...
{
  if (index >= cmbMouseWheelAction->count() - 1)
    plot->deregisterMouseWheelAction(Qt::NoModifier);
  else
    plot->registerMouseWheelAction(Qt::NoModifier,
                                   static_cast<JKQTPMouseWheelActions>(index));
}
...
void TestUserInteraction::plotMouseClicked(double x, double y,
                                           Qt::KeyboardModifiers modifiers,
                                           Qt::MouseButton button)
{
  labMouseClicked->setText(
    QString("plotMouseClicked(%1, %2, modifiers=%3, button=%4)")
      .arg(x)
      .arg(y)
      .arg(KeyboradMod2String(modifiers))
      .arg(MouseButton2String(button)));
}

void TestUserInteraction::plotMouseDoubleClicked(
  double x, double y, Qt::KeyboardModifiers modifiers, Qt::MouseButton button)
{
  labMouseClicked->setText(
    QString("plotMouseDoubleClicked(%1, %2, modifiers=%3, button=%4)")
      .arg(x)
      .arg(y)
      .arg(KeyboradMod2String(modifiers))
      .arg(MouseButton2String(button)));
...
void TestUserInteraction::plotNewZoomRectangle(double mouseDragRectXStart,
                                               ...
                                               Qt::KeyboardModifiers modifiers)
{
  labMouseAction->setText(
    QString("plotNewZoomRectangle(x=%1..%2, y=%3..%4, modifiers=%5)")
      .arg(mouseDragRectXStart)
      .arg(mouseDragRectXEnd)
      .arg(mouseDragRectYStart)
      .arg(mouseDragRectYEnd)
      .arg(KeyboradMod2String(modifiers)));
}

void TestUserInteraction::plotMouseWheelOperated(
  double x, double y, Qt::KeyboardModifiers modifiers, int deltaAngleX,
  int deltaAngleY)
{
  labMouseAction->setText(
    QString("plotMouseWheelOperated(x=%1, y%2, modifiers=%3, deltaAngleX=%4, "
            "deltaAngleY=%5)")
      .arg(x)
      .arg(y)
      .arg(KeyboradMod2String(modifiers))
...
void TestUserInteraction::contextMenuOpened(double x, double y,
                                            QMenu* contextMenu)
{
  if (contextMenu != plot->getSpecialContextMenu()) {
    // We only want to alter the standard context menu, not the special one, as
    // modifications there would be permanent! I.e. if this is called
    // repeatedly, entries will accumulate!
    //
    // If you want to modify the special menu, do not add actions, but use
    // pre-added actions, or reuse actions, once they have been added once!
    contextMenu->addSeparator();
    QAction* act =
      contextMenu
        ->addMenu(QString("contextMenuOpened(x=%1, y=%2)").arg(x).arg(y))
        ->addAction("user-added action");
    connect(act, &QAction::triggered, [x, y]() {
      QMessageBox::warning(
        nullptr, tr("Plot Context Menu"),
        tr("Context Menu was opened at x/y=%1/%2!").arg(x).arg(y),
        QMessageBox::Ok, QMessageBox::Ok);
    });
  }
  labMouseAction->setText(
    QString("contextMenuOpened(x=%1, y=%2)").arg(x).arg(y));
}

void TestUserInteraction::zoomChangedLocally(double newxmin, double newxmax,
                                             double newymin, double newymax,
                                             JKQTPlotter* /*sender*/)
{
  labMouseAction->setText(QString("zoomChangedLocally(x=%1..%2, y=%3..%4)")
                            .arg(newxmin)
                            .arg(newxmax)
                            .arg(newymin)
                            .arg(newymax));
...
void TestUserInteraction::userScribbleClick(double x, double y,
                                            Qt::KeyboardModifiers modifiers,
                                            bool first, bool last)
{
  ...
  if (first)
    ...
    counter++;
  labMouseAction->setText(
    QString("userClickFinished(%1, %2, modifiers=%3, first=%4, last=%5) -> "
            "scribble-points so far: %6")
      .arg(x)
      .arg(y)
      .arg(KeyboradMod2String(modifiers))
      .arg(first)
...
void TestUserInteraction::userRectangleFinished(double x, double y,
                                                double width, double height,
                                                Qt::KeyboardModifiers modifiers)
{
  labMouseAction->setText(
    QString(
      "userRectangleFinished(x=%1, y=%2, width=%3, height=%4, modifiers=%5)")
      .arg(x)
      .arg(y)
      .arg(width)
      .arg(height)
...
void TestUserInteraction::userLineFinished(double x1, double y1, double x2,
                                           double y2,
                                           Qt::KeyboardModifiers modifiers)
{
  labMouseAction->setText(
    QString("userLineFinished(x1=%1/%2, x2=%3/%4, modifiers=%5)")
      .arg(x1)
      .arg(y1)
      .arg(x2)
      .arg(y2)
...
void TestUserInteraction::userCircleFinished(double x, double y, double radius,
                                             Qt::KeyboardModifiers modifiers)
{
  labMouseAction->setText(
    QString("userCircleFinished(x=%1, y=%2, radius=%3, modifiers=%4)")
      .arg(x)
      .arg(y)
      .arg(radius)
      .arg(KeyboradMod2String(modifiers)));
...
void TestUserInteraction::userEllipseFinished(double x, double y,
                                              double radiusX, double radiusY,
                                              Qt::KeyboardModifiers modifiers)
{
  labMouseAction->setText(
    QString(
      "userEllipseFinished(x=%1, y=%2, radiusX=%3, radiusY=%4, modifiers=%5)")
      .arg(x)
      .arg(y)
      .arg(radiusX)
      .arg(radiusY)
...
void TestUserInteraction::tooltipDisplayed(
  double x, double y, const QStringList& entries,
  const QList<JKQTPPlotElement*>& graphs)
{
  labMouseAction->setText(QString("tooltipDisplayed(x=%1, y=%2, entries=%3)")
                            .arg(x)
                            .arg(y)
                            .arg(entries.join(";;")));
}

void TestUserInteraction::rulerDisplayed(double x1, double y1, double x2,
                                         double y2,
                                         Qt::KeyboardModifiers modifiers)
{
  labMouseAction->setText(
    QString("rulerDisplayed(x1=%1/%2, x2=%3/%4, modifiers=%5)")
      .arg(x1)
      .arg(y1)
      .arg(x2)
      .arg(y2)
...
void TestUserInteraction::initPlot()
{
  ...
  //    the variables columnX and columnY will contain the internal column ID of
  //    the newly created columns with names "x" and "y" and the (copied) data
  //    from X and Y.
