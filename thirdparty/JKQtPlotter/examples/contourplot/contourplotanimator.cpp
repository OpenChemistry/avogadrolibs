...
ContourPlotAnimator::ContourPlotAnimator(JKQTPDatastore* ds_,
                                         JKQTPlotter* pplot_, int NX_, int NY_,
                                         double w_, double h_, double dx_,
                                         size_t cPotential_)
  : QObject(pplot_), angle(0), ds(ds_), pplot(pplot_), NX(NX_), NY(NY_), w(w_),
    h(h_), dx(dx_), cPotential(cPotential_)
{
}
  ...
  for (size_t iy = 0; iy < static_cast<size_t>(NY); iy++) {
    ...
    for (size_t ix = 0; ix < static_cast<size_t>(NX); ix++) {
      ...
      ds->setPixel(cPotential, ix, iy,
                   Q1 / (4.0 * JKQTPSTATISTICS_PI * eps0) / xy_r1 +
                     Q2 / (4.0 * JKQTPSTATISTICS_PI * eps0) / xy_r2 +
                     Q3 / (4.0 * JKQTPSTATISTICS_PI * eps0) / xy_r3 +
                     Q4 / (4.0 * JKQTPSTATISTICS_PI * eps0) / xy_r4);
      x += dx;
    }
    y += dx;
  }
  pplot->redrawPlot();
  qDebug() << "animation step: angle=" << angle
           << ", framerate=" << 1.0 / (timerFR.elapsed() / 1000.0)
           << "fps <==> " << timerFR.elapsed() << "ms/frame";
  pplot->setWindowTitle("JKQTPColumnContourPlot [" +
                        QString::number(1.0 / (timerFR.elapsed() / 1000.0)) +
                        "fps]");
