...
int main(int argc, char* argv[])
{
  ...
  for (size_t iy = 0; iy < NY; iy++) {
    ...
    for (size_t ix = 0; ix < NX; ix++) {
      const double r1 =
        sqrt((x - Q1_x0) * (x - Q1_x0) + (y - Q1_y0) * (y - Q1_y0));
      const double r2 =
        sqrt((x - Q2_x0) * (x - Q2_x0) + (y - Q2_y0) * (y - Q2_y0));
      const double r3 =
        sqrt((x - Q3_x0) * (x - Q3_x0) + (y - Q3_y0) * (y - Q3_y0));
      const double r4 =
        sqrt((x - Q4_x0) * (x - Q4_x0) + (y - Q4_y0) * (y - Q4_y0));
  ...
  // the following code will plot a dipole JKQTPSTATISTICS_PIntial instead of
  // tJKQTPSTATISTICS_PIuadrupole after 5sJKQTPSTATISTICS_PI // this tests
  // theJKQTPSTATISTICS_PIognition of altered data
#ifdef ANIMATE
  ...
  // 3. create a graph (JKQTPColumnContourPlot) with the column created above as
  // data
  ...
  // add some levels for the contours. These are chosen to be at the actual
  // potential values at several specified relative distance from Q1, i.e. at
  // phi(Q1_x0*reldist) (phi: potential of Q1)
  QVector<double> reldists;
  reldists << 0.1 << 0.25 << 0.5 << 1 << 1.5 << 2 << 2.5 << 3;
  // finally contour levels with +1 and -1 sign are added to show the positive
  // and negative potential:
  for (auto reldist : reldists) {
    const double level =
      fabs(Q1 / (4.0 * JKQTPSTATISTICS_PI * eps0) / (Q1_x0 * reldist));
  ...
  // color contour lines from palette, but wothout taking their actual level
  // value into account:
