 ...
 * Several basic examples of how to add data to and edit data in a
 * JKQTPDatastore.
 *
...
int main(int argc, char* argv[])
{
  ...
  // 2. first we create data inside a QVector for a simple plot (a sine curve)
  // ... and add the plot
  ...
  // by calling JKQTPDatastore::addCopiedColumn() the data is COPIED from the
  // vector into the datastore
  linegraph->setXColumn(datastore->addCopiedColumn(X, "x"));
  linegraph->setYColumn(datastore->addCopiedColumn(Y, "y"));
  // alternatively you can also tell JKQTPDatastore to just reference an
  // external array:
...
// 3. Now we generate a plot from data in a C-array, just reference in the
// JKQTPDatastore
...
#define NDATA 5
  ...
  // linegraph->setXColumn(datastore->addCopiedColumn(XCA, NDATA, "xca
  // (C-array)")); linegraph->setYColumn(datastore->addCopiedColumn(YCA, NDATA,
  // "yca (C-array)"));
  linegraph->setTitle(QObject::tr("linked C-array data"));

  // 4. Since graphs often display (x,y)-pairs, it may make sense to store them
  // in a map (e.g. for histograms)
  //    There there are also functions that copy the contents of a map into a
  //    JKQTPDatastore, resulting in two columns beeing added:
  ...
  // 5. It is also possible to leave the data mangement completely to the
  // JKQTPDatastore
  //    and just edit the data with access functions from JKQTPDatastore:
  plot.addGraph(linegraph = new JKQTPXYLineGraph(&plot));
  //    5.1 this adds a column with 40 values, linearly spaced between 0 and 20
  //    (inclusive).
  size_t colLinX = datastore->addLinearColumn(40, 0, 20, "x_lin");
  //    5.2 this adds a column with one entry for every entry x in the column
  //    colLinX, where
  //        the entry is calculated by applying a function cos(x)*x/20.0
  size_t colFunc1 = datastore->addColumnCalculatedFromColumn(
    colLinX, [](double x) -> double { return cos(x) * x / 20.0; },
    "cos(x_lin)*x_lin/20.0");
  //        the same can be done by this code explicitly:
  //          5.2.1 add a column with as many rows as column colLinX
  // size_t colFunc1=datastore->addColumn(datastore->getRows(colLinX),
  // "cos(x_lin)*x_lin/20.0");
  //          5.2.2 iterate over the data in column colLinX and set a newly
  //          calculated value into a row of column colFunc1
  ...
  // 6. The function addLinearGridColumns() generates a rectangular 2D grid of
  // coordinates
  //    in row-major order. Here we generate a 10x10 grid with x-coordinates
  //    between 10 and 20 (inclusive) and y-coordinates between 1.5 and 3:
  std::pair<size_t, size_t> colLinXY =
    datastore->addLinearGridColumns(10, 10, 20, 10, 1.5, 3);
  // now we can add another column with 10*10=100 entries and fill it with some
  // values calculated from the the x and y-values in colLinXY:
  ...
  for (size_t i = 0; i < datastore->getRows(imgColumn); i++) {
    ...
    datastore->set(imgColumn, i,
                   cos((x - 15.0)) / (x - 15.0) * cos((y - 2.0)) / (x - 2.0));
  }
  ...
  //    }
  //}
  // the loop above can be written more compact using
  // addColumnCalculatedFromColumn():
  // imgColumn=datastore->addColumnCalculatedFromColumn(colLinXY.first,
  // colLinXY.second, [](double x, double y)->double { return
  // cos((x-15.0))/(x-15.0)*cos((y-2.0))/(x-2.0); },  "image value");
  // finally we can use a JKQTPXYParametrizedScatterGraph to display the data
  // from our three columns by using colLinXY->first and colLinXY->second as
  // positions for symbols that are colored, based on the respective value in
  // imgColumn:
  ...
  // alternatively you can only use the column imgColumn in a
