 ...
 * Shows how to plot Mathematical Functions as Line Graphs with JKQTPlotter (as
 * evaluated C/C++ functions)
 *
...
public:
  ...
  inline double operator()(double x) { return a * sin(x) * sin(x) / x / x; }
...
void drawExample(QApplication& app, const QString& name)
{
  ...
  //    and a JKQTPlotter to display the function, combine everything in a
  //    layout
  ...
  // 2. now we add a JKQTPXFunctionLineGraph object, which will draw a simple
  // function
  //    the function is defined as C++ inline function
  TFUNCGRAPH* func1 = new TFUNCGRAPH(plot);
  func1->setPlotFunctionFunctor(
    [](double x) { return 0.2 * x * x - 0.015 * x * x * x; });
  ...
  // 3. now we add a JKQTPXFunctionLineGraph object, which will draw a simple
  // function
  //    the function is again defined as C++ inline function, but now uses
  //    internal parameters (handed over to the function as a pointer to
  //    QVector<double>
  ...
  func2->setTitle("C++-inline function with int. params "
                  "$p_0\\cdot\\sin(x*2.0*\\pi\\cdot p_1)$");
  QObject::connect(chkShowSamples, &QCheckBox::toggled, [=](bool en) {
  ...
  // here we set offset p0=1 and slope p1=-1.5 of the line p0+p1*x by adding
  // these into a column in the internal datastore and then set that column as
  // parameterColumn for the function graph
...
int main(int argc, char* argv[])
{
  ...
  drawExample<JKQTPXFunctionLineGraph>(app,
                                       "functionplot: JKQTPXFunctionLineGraph");
  drawExample<JKQTPYFunctionLineGraph>(app,
                                       "functionplot: JKQTPYFunctionLineGraph");
