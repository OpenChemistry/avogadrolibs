 ...
 * Shows how to plot colored math images with JKQTPlotter, where different
 * images/matrices are assigned to different color channels
 *
...
int main(int argc, char* argv[])
{
  ...
  size_t cAiryDisk1 =
    ds->addCopiedImageAsColumn(airydisk1, NX, NY, "imagedata1");
  size_t cAiryDisk2 =
    ds->addCopiedImageAsColumn(airydisk2, NX, NY, "imagedata2");

  // 4. create a graph (JKQTPColumnMathImage) with the column created above as
  // data
  //    The data is color-coded with the color-palette JKQTPMathImageMATLAB
  //    the converted range of data is determined automatically because s
  //    etAutoImageRange(true)
  ...
  // set size of the data (the datastore does not contain this info, as it only
  // manages 1D columns of data and this is used to assume a row-major ordering
  graph->setNx(NX);
  graph->setNy(NY);
  ...
  graph->getColorBarRightAxisB()->setAxisLabel(
    "blue light field strength [AU]");
  graph->getColorBarRightAxisG()->setAxisLabel(
    "green light field strength [AU]");
  // determine min/max of data automatically and use it to set the range of the
  // color-scale
