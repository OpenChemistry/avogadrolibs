...
int main(int argc, char* argv[])
{
  ...
  // 3. create a graph (JKQTPImage) with a pointer to the QImage-object,
  // generated above
  JKQTPImage* graph = new JKQTPImage(&plot);
  graph->setTitle("");
  // copy the image into the graph (optionally you could also give a pointer to
  // a QImage, but then you need to ensure that the QImage is available as long
  // as the JKQTPImage instace lives)
  ...
  plot.getPlotter()->setAspectRatio(double(image.width()) /
                                    double(image.height()));
