...
int main(int argc, char* argv[])
{
  ...
  plotResid->synchronizeToMaster(plotMain, JKQTBasePlotter::sdXAxis, true, true,
                                 true);

  // 3.4 synchronize y-axis of width/plotResidHist to y-axis of width/plotResid
  plotResidHist->synchronizeToMaster(plotResid, JKQTBasePlotter::sdYAxis, true,
                                     true, true);

  // 3.5 ensure that the plot are printed/exported in whole, when printing in
  // plotMain
  plotMain->getPlotter()->setGridPrinting(true);
  plotMain->getPlotter()->addGridPrintingPlotter(0, 1, plotResid->getPlotter());
  plotMain->getPlotter()->addGridPrintingPlotter(1, 1,
                                                 plotResidHist->getPlotter());

  // 3.6 set relative sizes of the plots via the layout (small plots have 1/3
  // the width and height of the large plot
  ...
  //     - in plotResid we show the residuals, i.e. the difference between the
  //     curve and the datapoints
  //     - in plotResidHist we will show a histogram of the residuals
  //     (calculated in histogram)
  ...
  for (int n = 0; n < Ndata; ++n) {
    const double x = 3.0 * JKQTPSTATISTICS_PI / static_cast<double>(Ndata) *
  ...
  // 5.2 plot of the graph as an interpreted function, set as string
  // "2*(1+cos(x))"
  JKQTPXParsedFunctionLineGraph* graphFit =
    new JKQTPXParsedFunctionLineGraph(plotMain);
  ...
  JKQTPBarHorizontalGraph* graphResidHist =
    new JKQTPBarHorizontalGraph(plotResidHist);
  ...
  // 9. create a second window, which controls the synchronization and layout
  // between the plots
