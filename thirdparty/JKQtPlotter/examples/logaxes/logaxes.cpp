...
int main(int argc, char* argv[])
{
  ...
  // 2. now we create data a vector of x-values for a simple plot (resonance
  // curve with different damping value D)
  ...
  QVector<Qt::PenStyle> pens{ Qt::SolidLine, Qt::DashLine, Qt::DotLine,
  ...
  //    and is simply parametrized by a position (1.25/10) and the text to
  //    display. In addition you can also set the font size (here to 15) Use
  //    '$...$' around the actual math string to ensure rendering with a math
  //    font (the internal renderer uses XITS fonts by default, which are free
  //    and auto-distributed and loaded in the library). If you don't use the
  //    math-mode modifiers, the default font of the other rendering text is
  //    used, which might not be suitable for high-quality math rendering.
  plot.addGraph(
    new JKQTPGeoText(&plot, 1.25, 10,
                     "$\\frac{A}{A_{stat}}=\\frac{1}{\\sqrt{\\left(1-\\eta^"
                     "2\\right)^2+\\left(2{\\eta}D\\right)^2}}$",
                     15, QColor("black")));

  // 5. set y-axis to logarithmic (x-axis would be analogous, but using
  // `plot.getXAxis()`)
  plot.getYAxis()->setLogAxis(true);
  //    now we set the number of label ticks to 9 (you give the count if minor
  //    between two majors, so if you want ticks for
  //    1,2,3,...,10,20,30,...,100... you need to use 9:
  ...
  //    usually axis ticks are shown as numbers 0.01, 0.1, 1, 10, ... You can
  //    also force the scientific power-of-10 notation, using:
  plot.getYAxis()->setLabelType(JKQTPCALTexponent);
  //    the number of digits in JKQTPCALTexponent determines which labels are
  //    drawn in standard-notation, as compared to power-notation (e.g. if set
  //    to 1, the labels 0.1=10^{-1}, 1, 10 are shown in standard notation, the
  //    rest in power-notation. This tweak improves readability)
  plot.getYAxis()->setLabelDigits(0);
  // minor tick labels are usually not displayed, but you can switch them on,
  // using
  ...
  //    ... and finally set axis labels (using LaTeX notation and $...$ to
  //    improve rendering)
  plot.getYAxis()->setAxisLabel("Amplitude $A/A_{stat}$");
  plot.getXAxis()->setAxisLabel(
    "relative driving frequency $\\eta=\\omega/\\omega_0$");
