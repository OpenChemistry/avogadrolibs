 ...
 *
 * \ref JKQTPlotterAdvancedLineAndFillStyling, JKQTPXYLineGraph,
 * JKQTPSpecialLineHorizontalGraph, JKQTPBarVerticalGraph
 */
...
int main(int argc, char* argv[])
{
  ...
  for (int i = 0; i < Ndata; i++) {
    ...
    Yfilled2 << 0.75 *
                  (1.5 - 0.7 * x / 4.0 / JKQTPSTATISTICS_PI + sin(x * 1.5));
  ...
  // 3. now we add three semi-transparent, filled curve plots, one for each
  // histogram
  JKQTPSpecialLineHorizontalGraph* graphF =
    new JKQTPSpecialLineHorizontalGraph(&plot);
  JKQTPSpecialLineHorizontalGraph* graphF2 =
    new JKQTPSpecialLineHorizontalGraph(&plot);
  ...
  // 4. set data
  graphF->setXColumn(columnX);
  graphF->setYColumn(columnYFilled);
  graphF2->setXColumn(columnX);
  graphF2->setYColumn(columnYFilled2);
  graphBE->setXColumn(columnX);
  graphBE->setYColumn(columnYBarsEmpty);
  graphBF->setXColumn(columnX);
  graphBF->setYColumn(columnYBarsFilled);
  ...
  // 5.2 set advanced line style (see also
  ...
  // 5.3 set fill style, using a custom linear gradient, also with changing
  ...
  // 5.4 fill barcharts with transparencies and make the surrounding line
  // invisible (colored transparent)
