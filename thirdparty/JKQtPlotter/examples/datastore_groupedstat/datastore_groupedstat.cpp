 ...
 * jkqtptools_statistics ) together with JKQTPDatastore to generate grouped
 * statistics (i.e. calculates errorbars or boxplots from groups of datapoints
 * in a x/y-dataset).
 *
...
int main(int argc, char* argv[])
{
  ...
  plotscattererrors->getPlotter()->setPlotLabel(
    "Scatter Plot with Error Indicators");
  ...
  //      The datapoints consist of pairs <group,value>, where the groups are
  //      encoded by the numbers 1,2,3 and in each group, several measurements
  //      are taken
  ...
  //      library.
  //      First we need to group the data using jkqtpstatGroupData(), which
  //      assembles the data points in each group groupeddataBar
  std::map<double, std::vector<double>> groupeddataBar;
  jkqtpstatGroupData(datastore1->begin(colBarRawGroup),
                     datastore1->end(colBarRawGroup),
                     datastore1->begin(colBarRawValue),
                     datastore1->end(colBarRawValue), groupeddataBar);
  ...
  for (auto it = groupeddataBar.begin(); it != groupeddataBar.end(); ++it) {
    ...
    datastore1->appendToColumn(
      colBarAverage, jkqtpstatAverage(it->second.begin(), it->second.end()));
    datastore1->appendToColumn(
      colBarStdDev, jkqtpstatStdDev(it->second.begin(), it->second.end()));
  }
  ...
  // 2.5. With the methods above we can also calculate more advanced statistics,
  // like e.g. boxplots:
  ...
  for (auto it = groupeddataBar.begin(); it != groupeddataBar.end(); ++it) {
    datastore1->appendToColumn(
      colBarMedian, jkqtpstatMedian(it->second.begin(), it->second.end()));
    datastore1->appendToColumn(
      colBarMin, jkqtpstatMinimum(it->second.begin(), it->second.end()));
    datastore1->appendToColumn(
      colBarMax, jkqtpstatMaximum(it->second.begin(), it->second.end()));
    datastore1->appendToColumn(
      colBarQ25, jkqtpstatQuantile(it->second.begin(), it->second.end(), 0.25));
    datastore1->appendToColumn(
      colBarQ75, jkqtpstatQuantile(it->second.begin(), it->second.end(), 0.75));
  }
  // 2.6. The result can be plotted using JKQTPBoxplotVerticalGraph, which
  // receives a column for each value class of the final plot:
  ...
  // 2.7. In order to safe yo the typing of the code above, shortcuts in the
  // form of adaptors exist:
  /*
  jkqtpstatAddYErrorBarGraph(plotbarchart->getPlotter(),
                             datastore1->begin(colBarRawGroup),
  datastore1->end(colBarRawGroup), datastore1->begin(colBarRawValue),
  datastore1->end(colBarRawValue));
  ...
  */
  ...
  // 3.1. First we generate a second dataset, which is going to be used for a
  // scaterplot
  //      The datapoints consist of pairs <x,y>, that are based on a parabula
  //      with random deviations, both in x- and y-direction
  ...
  for (size_t i = 0; i < N; i++) {
    const double x = (static_cast<double>(i) - static_cast<double>(N) / 2.0) *
                     xmax / (static_cast<double>(N) / 2.0);
    const double y = jkqtp_sqr(x) + 2.0;
    datastore1->appendToColumns(colScatterRawX, colScatterRawY, x + d1(gen),
                                y + d1(gen));
  }
  // 3.2. Now we can also add the raw dataset to the plot for visualization:
  JKQTPXYParametrizedScatterGraph* gScatterRaw;
  plotscattererrors->addGraph(
    gScatterRaw = new JKQTPXYParametrizedScatterGraph(plotscattererrors));
  ...
  // 3.3. Now we want to draw a scatterchart of the data, where data-points
  // should be grouped
  //      together, in x-intervals of width 0.5. From all the points in each
  //      interval, we calculate the in both x- and y-direction the average and
  //      standard deviation. First we need to group the data using
  //      jkqtpstatGroupData(), which assembles the data points in each group
  //      groupeddataScatter. For the custom grouping of the datapoints we use
  //      the optional functor provided to jkqtpstatGroupData(): We use
  //      jkqtpstatGroupingCustomRound1D() with given parameters 0.25 for the
  //      (center) location of the first bin and bin width 0.5. The functor is
  //      not built by hand (which would be possible using std::bind), but with
  //      the generator function jkqtpstatMakeGroupingCustomRound1D(). in
  //      addition we use a variant of jkqtpstatGroupData(), which outputs a
  //      column with the category assigned to every data pair in the input data
  //      range
  std::map<double, std::pair<std::vector<double>, std::vector<double>>>
    groupeddataScatter;
  size_t colScatterRawGroup =
    datastore1->addColumn("scatterplot, rawdata, assigned-group");
  jkqtpstatGroupData(
    ...
    datastore1->backInserter(colScatterRawGroup), groupeddataScatter,
    jkqtpstatMakeGroupingCustomRound1D(0.25, 0.5));
  //       The column colScatterRawGroup can now be used to color the scatter
  //       graph:
  ...
  //       and calling functions like jkqtpstatAverage() for each group
  for (auto it = groupeddataScatter.begin(); it != groupeddataScatter.end();
       ++it) {
    datastore1->appendToColumn(
      colScatterXAvg,
      jkqtpstatAverage(it->second.first.begin(), it->second.first.end()));
    datastore1->appendToColumn(
      colScatterXStd,
      jkqtpstatStdDev(it->second.first.begin(), it->second.first.end()));
    datastore1->appendToColumn(
      colScatterYAvg,
      jkqtpstatAverage(it->second.second.begin(), it->second.second.end()));
    datastore1->appendToColumn(
      colScatterYStd,
      jkqtpstatStdDev(it->second.second.begin(), it->second.second.end()));
  }

  // 3.4. Finally the calculated groups are drawn
  JKQTPXYLineErrorGraph* gScatterErr;
  plotscattererrors->addGraph(gScatterErr =
                                new JKQTPXYLineErrorGraph(plotscattererrors));
  ...
  jkqtpstatAddXYErrorLineGraph(plotscattererrors->getPlotter(),
                               datastore1->begin(colScatterRawX),
  datastore1->end(colScatterRawX), datastore1->begin(colScatterRawY),
  datastore1->end(colScatterRawY), jkqtpstatMakeGroupingCustomRound1D(0.25,
  0.5));
  */
  ...
  plotboxplot->getPlotter()->setKeyBackgroundColor(
    QColorWithAlphaF("white", 0.25), Qt::SolidPattern);
  ...
  plotbarchart->getPlotter()->setKeyBackgroundColor(
    QColorWithAlphaF("white", 0.25), Qt::SolidPattern);
  ...
  plotscattererrors->getPlotter()->setKeyBackgroundColor(
    QColorWithAlphaF("white", 0.25), Qt::SolidPattern);
