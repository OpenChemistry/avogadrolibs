...
int main(int argc, char* argv[])
{
  ...
  int ipen = 0;
  for (int symbolID = 0; symbolID <= static_cast<int>(JKQTPMaxSymbolID);
       symbolID++) {
    // generate some plot data
    QVector<double> Y;
    const QVector<double>& X =
      (symbolID <= static_cast<int>(JKQTPMaxSymbolID) / 2) ? X1 : X2;
    const double dy =
      (symbolID <= static_cast<int>(JKQTPMaxSymbolID) / 2)
        ? (static_cast<double>(symbolID) * 1.5)
        : (static_cast<double>(symbolID -
                               static_cast<int>(JKQTPMaxSymbolID) / 2) *
           1.5);
    for (auto& xx : X) {
      Y << sin(xx) + dy;
    }
    ...
    graph->setTitle(JKQTPGraphSymbols2NameString(
                      static_cast<JKQTPGraphSymbols>(graph->getSymbolType())) +
                    QString(", ") + lname);
    ...
    JKQTPPlotSymbol(p,
    pix.width()/2,pix.height()/2,static_cast<JKQTPGraphSymbols>(symbolID),pix.width()/1.2,1.5,QColor("blue"),
    QColor("blue").lighter()); p.end();
    pix.save(JKQTPGraphSymbols2String(static_cast<JKQTPGraphSymbols>(symbolID))+".png",
    "PNG");*/
  }

  // 4. autoscale the plot so the graph is contained
  plot.getPlotter()->setKeyPosition(
    JKQTPKeyPosition::JKQTPKeyOutsideBottomLeft);
