...
bool LammpsDataFormat::write(std::ostream& outStream, const Core::Molecule& mol)
{
  ...
  if (numAtoms) {
    ...
    for (Index i = 0; i < numAtoms; ++i) {
      ...
      atomStream << std::setw(static_cast<size_t>(log(numAtoms)) + 1)
                 << std::left << to_string(i + 1) << " "
                 << to_string(composition[atomicNumbers[i]]) << "  "
                 << std::setw(10) << std::right << std::fixed << coords.x()
                 << "  " << std::setw(10) << std::right << std::fixed
                 << coords.y() << "  " << std::setw(10) << std::right
                 << std::fixed << coords.z() << "\n";
    }
  ...
  if (numBonds) {
    ...
    for (Index i = 0; i < numBonds; ++i) {
      Bond b = mol.bond(i);
      bondStream << b.atom1().index() + 1 << " " << b.atom2().index() + 1 << " "
                 << b.order() << endl;
    }
  }

  outStream << std::setw(10) << xmin - 0.5 << " " << std::setw(10) << xmax + 0.5
            << " xlo xhi" << endl
            << std::setw(10) << ymin - 0.5 << " " << std::setw(10) << ymax + 0.5
            << " ylo yhi" << endl
            << std::setw(10) << zmin - 0.5 << " " << std::setw(10) << zmax + 0.5
            << " zlo zhi" << endl;
