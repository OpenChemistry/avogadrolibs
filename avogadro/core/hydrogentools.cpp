                                         ...
                                         std::vector<size_t> &indices)
{
  ...
  signed char HydrogenTools::perceiveHybridization(const Atom &atom)
  {
    const NeighborListType bonds(atom.molecule()->bonds(atom));
    const unsigned int numberOfBonds(countExistingBonds(bonds)); // bond order sum

    int hybridization = 3; // default to sp3

    // TODO: Handle hypervalent species, SO3, SO4, lone pairs, etc.

    if (numberOfBonds > 4) {
      hybridization = numberOfBonds; // e.g., octahedral, trig. bipyr., etc.
    } else {
    ...
    std::vector<Vector3> &positions)
{
  ...
  signed char hybridization = atom.hybridization();
  if (hybridization == 0) {
    // Perceive it
    hybridization = perceiveHybridization(atom);
    //    atom.setHybridization(-1 * hybridization);
  } else if (hybridization < 0) { // perceived
    hybridization *= -1; // let's use the positive version
  }
  ...
  Vector3 HydrogenTools::generateNewBondVector(const Atom &atom, std::vector<Vector3> &allVectors, int hybridization)
  {
    Vector3 newPos;
    bool success = false;
    ...
    if (currentValence == 0) {
      ...
      // Check what's attached to our neighbor -- we want to set trans to the neighbor
      ...
      switch (hybridization) {
      case 1: // shouldn't happen, but maybe with metal atoms?
      case 2:
        newPos = v1;
      case 3:
      default:
