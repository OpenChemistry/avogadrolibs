...
SimuneSiestaInput::SimuneSiestaInput(QObject *parent_) :
  ...
  //m_savePath("")
{
  ...
  m_action->setText(tr("&SIMUNE SIESTA"));
  connect(m_action, SIGNAL(triggered()), SLOT(menuActivated()));
}

SimuneSiestaInput::~SimuneSiestaInput()
{
}

QList<QAction *> SimuneSiestaInput::actions() const
{
...
QStringList SimuneSiestaInput::menuPath(QAction *) const
{
  QStringList path;
...
void SimuneSiestaInput::setMolecule(QtGui::Molecule *mol)
{
  if (m_dialog)
...
void SimuneSiestaInput::openJobOutput(const JobObject &job)
{
  m_outputFormat = NULL;
...
bool SimuneSiestaInput::readMolecule(QtGui::Molecule &mol)
{
...
void SimuneSiestaInput::menuActivated()
{
  if (!m_dialog) {
    m_dialog = new SimuneSiestaInputDialog(qobject_cast<QWidget*>(parent()));
