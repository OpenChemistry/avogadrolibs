...
// Define this to draw debugging drawables to the scene.
#undef DEBUG_VECTORS
...
class Arc : public LineStripGeometry
{
public:
  Arc() {}
  ~Arc() {}
...
void Arc::setArc(const Vector3f &origin, const Vector3f &start,
                 const Vector3f &normal, float degreesCCW,
                 float resolutionDeg, float lineWidth)
{
  // Prepare rotation, calculate sizes
...
void BondCentricTool::draw(Rendering::GroupNode &node)
{
  ...
  case RotateNeighborAtom: {
    ...
    if (otherBond.isValid()) {
      /// @todo query scene metadata for largest rendered radius
      const float radius(static_cast<float>(
                           Elements::radiusVDW(anchorAtom.atomicNumber()))
                         * 0.3f // Match B&S radius
                         * 1.05f); // Make /slightly/ larger.
      drawBondAngle(*geo, selectedBond, otherBond, radius);
    }
    break;
  }
  }
  ...
  for (size_t i = 0; i < m_molecule->atomCount(); ++i) {
    Vector2i pos = camera.project(m_molecule->atom(i).position3d().cast<float>()).head<2>().cast<int>();
    Rendering::AbsoluteOverlayQuadStrategy *strat =
        new Rendering::AbsoluteOverlayQuadStrategy;
    strat->setAlign(Rendering::AbsoluteOverlayQuadStrategy::HCenter,
                    Rendering::AbsoluteOverlayQuadStrategy::VCenter);
    strat->setAnchor(pos);

    Rendering::TextProperties tprop;
    tprop.setPointSize(8);
    tprop.setColorRgb(255, 128, 128);

    Rendering::TextLabel *label = new Rendering::TextLabel;
    label->setQuadPlacementStrategy(strat);
    label->setTextProperties(tprop);
    label->setRenderPass(Rendering::OverlayPass);
    label->setString((QString::number(pos.x()) + " " + QString::number(pos.y())).toStdString());
...
void BondCentricTool::initializeBondVectors()
{
  ...
  if (bond.isValid()) {
    ...
    m_planeNormal = m_bondVector.unitOrthogonal();
  }
}
...
QUndoCommand *BondCentricTool::rotatePlane(QMouseEvent *e)
{
  ...
  // Rotate
  m_planeNormal = rotator * m_planeNormal;
  emit drawablesChanged();
...
#ifdef DEBUG_VECTORS
  // Debugging:
  LineStripGeometry *vectors = new LineStripGeometry;
  node.addDrawable(vectors);
  vectors->setRenderPass(Rendering::OpaquePass);

  Array<Vector3f> vectorArray;
  vectorArray.push_back(atom1Pos);
  vectorArray.push_back(atom1Pos + m_bondVector);

  vectors->setColor(Vector3ub(255, 0, 0));
  vectors->addLineStrip(vectorArray, 5.0);

  vectorArray[1] = atom1Pos + normal;
  vectors->setColor(Vector3ub(0, 255, 0));
  vectors->addLineStrip(vectorArray, 5.0);

  vectorArray[1] = atom1Pos + offset;
  vectors->setColor(Vector3ub(0, 255, 255));
  vectors->addLineStrip(vectorArray, 5.0);
#endif
}
                                    ...
                                    float radius) const
{
  ...
  Vector3f startEdge(newA2Direction);
  ...
  // The origin + ave(arcStart, arcStop):
  const Vector3f textPos(a1 + (startEdge + selectedBondOffset) * 0.5f);

  Rendering::TextLabel *label = new Rendering::TextLabel;
  label->setString(QString::number(displayAngle, 'f', 2).toStdString());
                                          ...
                                          const Core::Bond &bond)
{
  ...
  Rendering::TextLabel *label = new Rendering::TextLabel;
  label->setString(QString::number(bondVector.norm(), 'f', 4).toStdString());
  label->setRenderPass(Rendering::Overlay3DPass);
  node.addDrawable(label);
                                         ...
                                         const Bond &anchorBond)
{
  ...
  while (bondIter != bondEnd) {
    if (*bondIter != anchorBond)
      drawAtomBondAngle(node, atom, anchorBond, *bondIter);
    ++bondIter;
  }
}
                                        ...
                                        const Core::Bond &otherBond)
{
  ...
  const Vector3f otherVector(
        (otherAtom.position3d() - atom.position3d()).cast<float>());
  const Vector3f anchorVector(
        (otherAnchorAtom.position3d() - atom.position3d()).cast<float>());
  /// @todo Add atom radius metadata to scene.
  const float radius(static_cast<float>(
                       Elements::radiusVDW(atom.atomicNumber()))
                     * 0.3f // Match B&S radius
                     * 1.05f); // Make /slightly/ larger.

  const Vector3f origin(atom.position3d().cast<float>());
  const Vector3f start(anchorVector.normalized() * radius);
...
inline void BondCentricTool::transformFragment() const
{
  for (std::vector<int>::const_iterator
    ...
    if (atom.isValid()) {
      Vector3f pos = atom.position3d().cast<float>();
      pos = m_transform * pos;
      atom.setPosition3d(pos.cast<Real>());
    }
  }
}
