...
public:
  FileDialogModelFileInfo():
    m_type(FileDialogModel::INVALID),
    m_hidden(false)
  {
  }

  FileDialogModelFileInfo(const QString& l, const QString& filepath,
           FileDialogModel::FileType t, const bool &h,
  ...
  FileDialogModel::FileType type() const
  {
    return m_type;
  }
...
public:
    ...
    m_model(model)
  {
    ...
    Closure *callback = NewCallback(this,
        &FileDialogModel::Private::handleSeparatorResponse,
        separator);
  ...
  void listDirectory(Path &dir) {
    ...
    Closure *callback = NewCallback(this,
        &FileDialogModel::Private::handleListingResponse,
        listing);
  ...
  void handleListingResponse(Listing *response)
  {
    ...
    for(int i=0; i< response->paths_size(); i++) {
      ...
      FileDialogModelFileInfo info(label, label,
          static_cast<FileDialogModel::FileType>(path.type()), false);
...
FileDialogModel::FileDialogModel(
  ...
  base(Parent),
  m_implementation(new Private(this, _server))
{
}
    ...
    const QObject *requester, const char* resultSlot)
{
  ...
  AbsoluteFilePathRequest *request = new AbsoluteFilePathRequest(server(),
      m_implementation->m_currentPath,  path);

  connect(request, SIGNAL(complete(const QString &)),
    ...
    const char *resultSlot)
{
  ...
  connect(request, SIGNAL(complete(const QString &, bool)),
    requester, resultSlot);
  connect(request, SIGNAL(complete(const QString &, bool)),
      this, SLOT(cleanup()));
...
public:
  ...
  void handleDirExists(Listing *listing)
  {
    if (FileDialogModel::isDirectory(
      ...
      emit complete(QString::fromStdString(listing->path().path()), true);
    else if (!FileDialogModel::isFile(
        static_cast<FileDialogModel::FileType>(listing->path().type()))
...
int FileDialogModel::columnCount(const QModelIndex& idx) const
{
  const FileDialogModelFileInfo* file =
    m_implementation->infoForIndex(idx);
