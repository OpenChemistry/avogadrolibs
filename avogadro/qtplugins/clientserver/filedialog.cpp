...
FileDialog::FileDialog(vtkCommunicatorChannel* server, QWidget* p,
    ...
    const QString& nameFilter) : QDialog(p),
  m_implementation(new Private(this, server))
{
  ...
  QObject::connect(m_implementation->m_ui.NavigateBack,
                   SIGNAL(clicked(bool)),
  ...
  m_implementation->m_ui.NavigateUp->setIcon(style()->
  ...
  QObject::connect(m_implementation->m_model,
                   SIGNAL(modelReset()),
                   this,
                   SLOT(onModelReset()));

  QObject::connect(m_implementation->m_ui.NavigateUp,
                   SIGNAL(clicked()),
                   this,
                   SLOT(onNavigateUp()));
  ...
  QObject::connect(m_implementation->m_ui.Files->selectionModel(),
                  SIGNAL(
...
void FileDialog::onContextMenuRequested(const QPoint &menuPos)
{
  ...
  actionHiddenFiles->setChecked(
      m_implementation->m_fileFilter.getShowHidden());
...
public slots:
  ...
  void accept(const QString &file) {
    ...
    m_dialog->m_implementation->m_model->dirExists(file, this,
        SLOT(onDirExists(const QString, bool)));
  }
  ...
  void acceptFile(const QString &file) {
    m_dialog->m_implementation->m_model->fileExists(file, this,
        SLOT(onFileExistsComplete(const QString, bool)));
  }
  ...
  void acceptOnDirExists(const QString &dir, bool exists) {
    if (exists) {
      ...
      m_dialog->m_implementation->m_model->fileExists(m_selectedFile, this,
...
void FileDialog::acceptExistingFiles()
{
  ...
  if (m_implementation->m_fileName.isEmpty()) {
    ...
    AcceptRequest *request = new AcceptRequest(this,
        m_implementation->m_fileName, false);

    connect(request, SIGNAL(finished(bool)),
        this, SLOT(acceptRequestFinished(bool)));

    m_implementation->m_model->absoluteFilePath(
        m_implementation->m_fileName, request,
        SLOT(accept(const QString&)));
...
void FileDialog::acceptDefault()
{
  ...
  m_implementation->m_model->absoluteFilePath(filename,
  ...
  if (!ext.isEmpty()) {
    ...
    foreach (QString wildcard, wildCards) {
      if (wildcard.indexOf('.') != -1) {
        ...
        QRegExp regEx = QRegExp(wildcard, Qt::CaseInsensitive,
            QRegExp::Wildcard);
