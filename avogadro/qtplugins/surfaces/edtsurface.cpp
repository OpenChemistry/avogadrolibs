...
namespace Avogadro {
  ...
  void EDTSurface::fastOneShell(int* inNum, int* allocOut,
                                Vector3i*** boundPoint, int* outNum, int* elimi)
  {
    int i, number;
    Vector3i dxyz;
    Vector3i txyz;
    ...
    for (i = 0; i < number; i++) {
      ...
      for (j = 0; j < 6; j++) {
        ...
        if (inBounds(tnv) && _inOut[tnv(X)][tnv(Y)][tnv(Z)] &&
          ...
          qDebug() << " set1 " << tnv[0] << tnv[1] << tnv[2] << dxyz.norm();
          ...
          if (squre < m_cube->value(tnv)) {
            boundPoint[tnv(X)][tnv(Y)][tnv(Z)] =
              boundPoint[txyz(X)][txyz(Y)][txyz(Z)];
            m_cube->setValue(tnv, dxyz.norm());
            qDebug() << " set2 " << tnv[0] << tnv[1] << tnv[2] << dxyz.norm();
    ...
    for (i = 0; i < number; i++) {
      ...
      for (j = 6; j < 18; j++) {
        ...
        if (inBounds(tnv) && _inOut[tnv(X)][tnv(Y)][tnv(Z)] &&
          ...
          dxyz = tnv - boundPoint[txyz(X)][txyz(Y)][txyz(Z)];
          m_cube->setValue(tnv, dxyz.norm());
          qDebug() << " set3 " << tnv[0] << tnv[1] << tnv[2] << dxyz.norm();
          _isDone[tnv(X)][tnv(Y)][tnv(Z)] = true;
          _isBound[tnv(X)][tnv(Y)][tnv(Z)] = true;
          data->outArray[data->positOut] = tnv;
          data->positOut++;
          ...
          if (squre < m_cube->value(tnv)) {
            ...
            qDebug() << " set4 " << tnv[0] << tnv[1] << tnv[2] << float(squre);
            if (!_isBound[tnv(X)][tnv(Y)][tnv(Z)]) {
              _isBound[tnv(X)][tnv(Y)][tnv(Z)] = true;
              data->outArray[data->positOut] = tnv;
              data->positOut++;
    ...
    for (i = 0; i < number; i++) {
      ...
      for (j = 18; j < 26; j++) {
        ...
        if (inBounds(tnv) && _inOut[tnv(X)][tnv(Y)][tnv(Z)] &&
          ...
          dxyz = tnv - boundPoint[txyz(X)][txyz(Y)][txyz(Z)];
          m_cube->setValue(tnv, dxyz.norm());
          qDebug() << " set5 " << tnv[0] << tnv[1] << tnv[2] << dxyz.norm();
          _isDone[tnv(X)][tnv(Y)][tnv(Z)] = true;
          _isBound[tnv(X)][tnv(Y)][tnv(Z)] = true;
          data->outArray[data->positOut] = tnv;
          data->positOut++;
  ...
  bool EDTSurface::inBounds(Vector3i vec)
  {
    return (vec(X) > -1 && vec(Y) > -1 && vec(Z) > -1 &&
            vec(X) < data->pLength && vec(Y) < data->pWidth &&
            vec(Z) < data->pHeight);
  }
