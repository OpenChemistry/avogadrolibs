...
SimuneAntInput::SimuneAntInput(QObject *parent_) :
  ...
  //m_savePath("")
{
  ...
  m_action->setText(tr("&SIMUNE ANT.G"));
  connect(m_action, SIGNAL(triggered()), SLOT(menuActivated()));
}

SimuneAntInput::~SimuneAntInput()
{
}

QList<QAction *> SimuneAntInput::actions() const
{
...
QStringList SimuneAntInput::menuPath(QAction *) const
{
  QStringList path;
...
void SimuneAntInput::setMolecule(QtGui::Molecule *mol)
{
  if (m_dialog)
...
void SimuneAntInput::openJobOutput(const JobObject &job)
{
  m_outputFormat = NULL;
...
bool SimuneAntInput::readMolecule(QtGui::Molecule &mol)
{
...
void SimuneAntInput::menuActivated()
{
  if (!m_dialog) {
    m_dialog = new SimuneAntInputDialog(qobject_cast<QWidget*>(parent()));
