...
PlotBands::PlotBands(QObject *parent_) :
...
QList<QAction *> PlotBands::actions() const
{
...
QStringList PlotBands::menuPath(QAction *) const
{
  QStringList path;
...
void PlotBands::setMolecule(QtGui::Molecule *mol)
{
  if (m_dialog)
...
void PlotBands::openJobOutput(const JobObject &job)
{
  m_outputFormat = NULL;
...
bool PlotBands::readMolecule(QtGui::Molecule &mol)
{
...
void PlotBands::menuActivated()
{
  if (!m_dialog) {
    m_dialog = new PlotBandsDialog(qobject_cast<QWidget*>(parent()));
    //m_dialog = new PlotBandsDialog();
