...
SymmetryScene::SymmetryScene(QObject *p) : QtGui::ScenePlugin(p), m_enabled(true)
{
...
void SymmetryScene::process(const Core::Molecule &coreMolecule, Rendering::GroupNode &node)
{
  qDebug() << "SymmetryScene::process";

  const QtGui::Molecule *molecule = dynamic_cast<const QtGui::Molecule*>(&coreMolecule);
  if (!molecule)
    return;
  ...
  QVariant properRotation =  molecule->property("SymmetryProperRotationVariantList");
  QVariant improperRotation =  molecule->property("SymmetryImproperRotationVariantList");
  ...
  if(properRotation.isValid()){
    ...
    foreach(QVariant qv, properRotationVariantList){
      ...
      Vector3f fvec = Vector3f(qvec.x(),qvec.y(),qvec.z());
      cylinders->addCylinder(forigo,forigo+1.1*fradius*fvec,0.05f,color);
    }
  }
  ...
  if(reflection.isValid()){
    ...
    foreach(QVariant qv, reflectionVariantList){
      QVector3D qvec = qv.value<QVector3D>();
      Vector3f fvec = Vector3f(qvec.x(),qvec.y(),qvec.z());
      cylinders->addCylinder(forigo-fvec*0.025f,forigo+fvec*0.025f,fradius,color);
    }
  }
