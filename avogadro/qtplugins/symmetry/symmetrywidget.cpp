  ...
  m_radius(0.0)
{
  ...
  m_ui->operationsTable->setItemDelegateForColumn(OperationsTableModel::ColumnType, new RichTextDelegate(this));

  m_ui->subgroupsTree->setModel(m_subgroupsTreeModel);
  m_ui->subgroupsTree->setItemDelegateForColumn(0,new RichTextDelegate(this));

  connect(m_ui->detectSymmetryButton, SIGNAL(clicked()), SIGNAL(detectSymmetry()));
  connect(m_ui->symmetrizeMoleculeButton, SIGNAL(clicked()), SIGNAL(symmetrizeMolecule()));
  connect(m_ui->operationsTable->selectionModel(),SIGNAL(selectionChanged(const QItemSelection &, const QItemSelection &)),SLOT(operationsSelectionChanged(const QItemSelection &, const QItemSelection &)));
  connect(m_ui->subgroupsTree->selectionModel(),SIGNAL(selectionChanged(const QItemSelection &, const QItemSelection &)),SLOT(subgroupsSelectionChanged(const QItemSelection &, const QItemSelection &)));
...
void SymmetryWidget::operationsSelectionChanged(const QItemSelection & selected, const QItemSelection & deselected)
{
  ...
  QModelIndexList selection = m_ui->operationsTable->selectionModel()->selectedRows();
  ...
  foreach(QModelIndex i, selection){
    ...
    if(!i.isValid() || row >= m_sopsl) continue;
    float x = m_sops[row].v[0], y = m_sops[row].v[1], z = m_sops[row].v[2];
    switch(m_sops[row].type){
      case IDENTITY : break;
      case PROPER_ROTATION : properRotationVariantList.append(QVector3D(x,y,z)); break;
      case IMPROPER_ROTATION : improperRotationVariantList.append(QVector3D(x,y,z)); break;
      case REFLECTION : reflectionVariantList.append(QVector3D(x,y,z)); break;
      case INVERSION : m_molecule->setProperty("SymmetryInversion", m_cm); break;
      default : break;
    }
  }
  if(properRotationVariantList.size() > 0)
    m_molecule->setProperty("SymmetryProperRotationVariantList", properRotationVariantList);
  if(improperRotationVariantList.size() > 0)
    m_molecule->setProperty("SymmetryImproperRotationVariantList", improperRotationVariantList);
  if(reflectionVariantList.size() > 0)
    m_molecule->setProperty("SymmetryReflectionVariantList", reflectionVariantList);
...
{
  //QModelIndexList selection = m_ui->subgroupsTree->selectionModel()->selectedIndexes();
  QModelIndex i = m_ui->subgroupsTree->selectionModel()->selectedIndexes().first();
  qDebug() << "subgroupsSelectionChanged";
  if(!i.isValid()) return;
  qDebug() << "valid";
  int sgi = i.data(Qt::UserRole).value<int>();
  qDebug() << "index " << sgi;
  if(sgi < 0 || sgi >= m_sgl) return;
...
void SymmetryWidget::setSymmetryOperations(int sopsl, msym::msym_symmetry_operation_t *sops){
  m_sops = sops;
  m_sopsl = sopsl;
