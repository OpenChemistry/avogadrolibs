  ...
  m_viewSymmetryAction(new QAction(this))
{
  ...
  connect(m_symmetryWidget, SIGNAL(symmetrizeMolecule()), SLOT(symmetrizeMolecule()));
  /*
  ...
  connect(m_ui->toleranceCombo, SIGNAL(currentIndexChanged(int)), this, SLOT(toleranceChanged(int)));*/
...
/*
  ...
  if((changes & Molecule::Atoms) && (changes & Molecule::Modified || changes & Molecule::Added || changes & Molecule::Removed))
    detectSymmetry();
}
...
void Symmetry::detectSymmetry()
{
  ...
  if (m_molecule == NULL || m_molecule->atomPositions3d().size() != length || length < 2)
