                                    ...
                                    const Core::Molecule &mol) const
{
  ...
  if (m_inputMoleculeFormat == Unknown)
    options();

  if (m_inputMoleculeFormat == NoInputFormat)
    return true;
  ...
  default:
  case Unknown:
    m_errors << tr("InputGenerator::insertMolecule: Internal error. Cannot "
                   "determine input molecule format.");
    return false;
  case CJSON:
    key = "cjson";
    /// @todo update this when the new format class is written. Add error handling.
    Io::CjsonFormat().writeString(value, mol);
    break;
  }

  json.insert(key, QJsonValue(QString::fromStdString(value)));
  return true;
}
