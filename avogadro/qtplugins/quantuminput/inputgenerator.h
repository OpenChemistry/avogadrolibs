...
private:
  ...
  QByteArray execute(const QStringList &args, const QByteArray &stdin) const;
  QByteArray execute(const QStringList &args) const
  {
    // MSVS complains about using QByteArray() as a default arg in the other
    // signature.
    return execute(args, QByteArray());
  }
