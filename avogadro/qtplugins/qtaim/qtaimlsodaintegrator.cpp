...
QVector3D QTAIMLSODAIntegrator::integrate(QVector3D x0y0z0)
{
  ...
  for (t0 = 0.0; t0 < tf; t0 = t0 + dt) {
    ...
    if (istate <= 0) {
      ...
      //        qDebug(" at t= %12.4e y= %14.6e %14.6e %14.6e",
      //        t,y[1],y[2],y[3]);
...
void QTAIMLSODAIntegrator::dgesl(double** a, int n_, int* ipvt_, double* b,
                                 int job)
...
void QTAIMLSODAIntegrator::successreturn(double* y, double* t, int itask,
...
void QTAIMLSODAIntegrator::lsoda(int neq, double* y, double* t, double tout,
                                 int itol, double* rtol, double* atol,
                                 int itask, int* istate, int iopt, int jt,
                                 int iwork1, int iwork2, int iwork5, int iwork6,
                                 int iwork7, int iwork8, int iwork9,
                                 double rwork1, double rwork5, double rwork6,
                                 double rwork7)
/*
...
*/
{
  ...
  int i, iflag, lenyh;
  double atoli, ayi, big, hmax, hmx, rh, rtoli, tdist, tnext, tol, tolsf, tp,
    size, sum, w0;
    ...
    switch (itask) {
      case 1:
        if ((tn - tout) * h >= 0.) {
          ...
          if (iflag != 0) {
            qDebug("lsoda -- trouble from intdy, itask = %d, tout = %g", itask,
                   tout);
            terminate(istate);
            freevectors();
            return;
  ...
  while (1) {
    ...
    if ((tn + h) == tn) {
      ...
      if (nhnil <= mxhnil) {
        ...
        if (nhnil == mxhnil) {
          qDebug("lsoda -- above warning has been issued %d times,", nhnil);
...
void QTAIMLSODAIntegrator::correction(int neq, double* y, int* corflag,
                                      double pnorm, double* del, double* delp,
                                      double* told, int* ncf, double* rh,
                                      int* m)
/*
...
void QTAIMLSODAIntegrator::corfailure(double* told, double* rh, int* ncf,
                                      int* corflag)
{
  int j, i1, i;
...
void QTAIMLSODAIntegrator::methodswitch(double dsm, double pnorm, double* pdh,
                                        double* rh)
{
...
void QTAIMLSODAIntegrator::orderswitch(double* rhup, double dsm, double* pdh,
