...
void QTAIMODEIntegrator::r8_fehl(qint64 neqn, qreal y[], qreal t, qreal h_,
                                 qreal yp[], qreal f1[], qreal f2[], qreal f3[],
...
qint64 QTAIMODEIntegrator::r8_rkf45(qint64 neqn, qreal y[], qreal yp[],
                                    qreal* t, qreal tout, qreal* relerr,
...
//
//    Input, qint64 FLAG, indicator for status of integration. On the first
//    call,
...
#define MAXNFE 1000
  ...
  //
  //  If we are too close to the output point, then simply extrapolate and
  //  return.
  //
  ...
  for (;;) {
    ...
    //
    //  Will the next successful step complete the integration to the output
    //  point?
    //
    {
    ...
    for (;;) {
      ...
      for (k = 0; k < neqn; k++) {
        ...
        ee = r8_abs((-2090.0 * yp[k] + (21970.0 * f3[k] - 15048.0 * f4[k])) +
