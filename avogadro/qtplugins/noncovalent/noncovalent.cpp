...
NonCovalent::NonCovalent(QObject* p) : ScenePlugin(p)
{
  ...
  auto hydrogenBColor =
    settings.value("nonCovalent/lineColor0", QColor(64, 192, 255))
      .value<QColor>();
  auto halogenBColor =
    settings.value("nonCovalent/lineColor1", QColor(128, 255, 64))
      .value<QColor>();
  auto chalcogenBColor =
    settings.value("nonCovalent/lineColor2", QColor(255, 192, 64))
      .value<QColor>();
  m_lineColors = { Vector3ub(hydrogenBColor.red(), hydrogenBColor.green(),
                             hydrogenBColor.blue()),
                   Vector3ub(halogenBColor.red(), halogenBColor.green(),
                             halogenBColor.blue()),
                   Vector3ub(chalcogenBColor.red(), chalcogenBColor.green(),
                             chalcogenBColor.blue()) };
  m_lineWidths = { settings.value("nonCovalent/lineWidth0", 5.0).toFloat(),
...
static enum InteractionTypes getInteractionType(const Molecule& molecule,
                                                Index i)
{
...
static bool checkPairDonorIsValid(const Molecule& molecule, Index n,
                                  int interactionType)
{
...
static bool checkPairVector(const Molecule& molecule, Index n,
                            const Vector3& in, float angleTolerance)
{
  AtomHybridization hybridization =
    AtomUtilities::perceiveHybridization(molecule.atom(n));
...
void NonCovalent::process(const Molecule& molecule, Rendering::GroupNode& node)
{
  ...
  float absoluteMaxDistance =
    *std::max_element(m_maximumDistances.begin(), m_maximumDistances.end());
  ...
  for (Index i : enabledAtoms) {
    ...
    for (Index ni : neighbors) {
      ...
      if (distance_vector.norm() >
          m_maximumDistances[interactionType] + radius + nradius)
        continue;
      if (!checkAtomPairNotBonded(molecule, i, n))
        continue;

      float angleTolerance =
        m_angleTolerancesDegrees[interactionType] * M_PI / 180.0;
...
QWidget* NonCovalent::setupWidget()
{
  ...
  for (Index i = 0; i < 3; i++) {
    ...
    QObject::connect(lineWidth_spin,
...
void NonCovalent::setAngleTolerance(float angleTolerance, Index index)
{
  ...
  QSettings settings;
  settings.setValue(QString("nonCovalent/angleTolerance%1").arg(index),
                    angleTolerance);
}
...
void NonCovalent::setMaximumDistance(float maximumDistance, Index index)
{
  ...
  QSettings settings;
  settings.setValue(QString("nonCovalent/maximumDistance%1").arg(index),
                    maximumDistance);
}
