...
#endif
{
  ...
  if (!obabelExec.isEmpty()) {
    ...
    if (!baseDir.absolutePath().startsWith("/usr/") &&
      ...
      env.insert("BABEL_DATADIR",
                 QCoreApplication::applicationDirPath() + "/../share/openbabel/2.3.2");
      env.insert("BABEL_LIBDIR",
                 QCoreApplication::applicationDirPath() + "/../lib/openbabel/2.3.2");
#endif
      m_process->setProcessEnvironment(env);
    }
    else
      qDebug() << "FAIL:" << baseDir.absolutePath() + '/' + m_obabelExecutable;
  }
  qDebug() << "obabel executable:" << m_obabelExecutable;
}
