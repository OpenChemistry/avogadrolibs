...
void OpenBabel::onOptimizeGeometry()
{
  ...
  if (!m_progress)
    m_progress = new QProgressDialog(qobject_cast<QWidget*>(parent()));
  m_progress->setLabelText(tr("Optimizing geometry via OpenBabel:\n\n%1")
                           .arg(tr("Generating CML...")));
  m_progress->setRange(0, 0);
  m_progress->setMinimumDuration(0);
  m_progress->setValue(0);
  ...
  m_progress->setLabelText(tr("Optimizing geometry via OpenBabel:\n\n%1")
                           .arg(tr("Running %1...",
                                   "arg is an executable file.")
                                .arg(m_process->obabelExecutable())));

  // Run obabel
  /// @todo progress updates from obabel --minimize --log
