...
class RWMolecule;
struct AVOGADROQTGUI_EXPORT RWAtom
{
  RWAtom() : m_molecule(NULL), m_index(MaxIndex) {}
  RWAtom(RWMolecule *m, Index i) : m_molecule(m), m_index(i) {}
  RWMolecule* molecule() const { return m_molecule; }
  Index index() const { return m_index; }

  RWMolecule *m_molecule;
  Index m_index;
};
struct AVOGADROQTGUI_EXPORT RWBond
{
  RWBond() : m_molecule(NULL), m_index(MaxIndex) {}
  RWBond(RWMolecule *m, Index i) : m_molecule(m), m_index(i) {}
  RWMolecule* molecule() const { return m_molecule; }
  Index index() const { return m_index; }
...
public:
  ...
  AtomType addAtom(unsigned char atomicNumber, Index uniqueId); // TODO is this really needed?
  ...
  BondType addBond(Index atom1, Index atom2, unsigned char order,
                   Index uniqueId);  // TODO is this really needed?
  BondType addBond(const AtomType &atom1, const AtomType &atom2,
                   unsigned char order = 1);
  BondType addBond(const AtomType &atom1, const AtomType &atom2,
                   unsigned char order, Index uniqueId); // TODO is this really needed?
...
{
  return atomId < m_atomicNumbers.size() ? m_atomicNumbers[atomId] : 0;
}
...
inline RWMolecule::BondType
RWMolecule::addBond(const RWMolecule::AtomType &atom1,
                    const RWMolecule::AtomType &atom2,
                    unsigned char order, Index uniqueId)
{
  if (atom1.molecule() != atom2.molecule() || atom1.molecule() != this)
    return BondType();
  return addBond(atom1.index(), atom2.index(), order, uniqueId);
}
