...
struct RemapAtomicNumbers
{
  ...
  const std::map<unsigned char, unsigned char> &map;
...
void CustomElementDialog::apply()
{
  ...
  foreach (QComboBox* combo, findChildren<QComboBox*>()) {
    ...
    if (currentIndex == 0) {
      ...
      unsigned char newId = static_cast<unsigned char>(combo->currentIndex());
      oldToNew.insert(std::make_pair(oldId, newId));
    }
  }
...
void CustomElementDialog::accept()
{
  // Ensure that each edit combo box is not empty.
  foreach (QComboBox* combo, m_ui->form->findChildren<QComboBox*>()) {
    if (combo->currentIndex() > m_elements.size()
        && combo->currentText().trimmed().isEmpty()) {
      return; // cancel
    }
  }
