...
signals:
  void jobUpdated(Avogadro::QtGui::BatchJob::BatchId batchId, bool success);
  void jobCompleted(Avogadro::QtGui::BatchJob::BatchId batchId,
                    Avogadro::QtGui::BatchJob::JobState status);
...
inline float BatchJob::progress() const
{
  ...
  for (QVector<JobState>::const_iterator it = m_states.begin(),
    ...
    case Rejected:
    default:
      // Don't count these towards the progress.
      total -= numStages;
      break;
    case None:
    case Accepted:
    ...
    case Submitted:
    case QueuedRemote:
      // Jobs are staged and waiting to run.
      complete += 1;
      break;
    case RunningLocal:
    case RunningRemote:
  ...
  return total != 0 ? static_cast<float>(complete) / static_cast<float>(total)
                    : 1.f;
}
