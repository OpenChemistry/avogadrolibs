...
RWMolecule::AtomType RWMolecule::addAtom(unsigned char num, Index uid)
{
  if (uid >= m_atomUniqueIds.size() || m_atomUniqueIds[uid] != MaxIndex)
    return AtomType();
  Index atomId = static_cast<Index>(m_atomicNumbers.size());
  AddAtomCommand *comm = new AddAtomCommand(*this, num, atomId, uid);
  m_undoStack.push(comm);
  return AtomType(this, atomId);
}
...
namespace {
class SetPositions3dCommand : public RWMolecule::UndoCommand
{
...
namespace {
class SetPosition3dCommand : public RWMolecule::UndoCommand
{
  Index m_atomId;
  Vector3 m_oldPosition3d;
  Vector3 m_newPosition3d;
public:
  SetPosition3dCommand(RWMolecule &m, Index atomId,
    ...
    : UndoCommand(m),
      m_atomId(atomId),
      m_oldPosition3d(oldPosition3d),
      m_newPosition3d(newPosition3d)
  {
  }

  void redo() AVO_OVERRIDE
  {
    positions3d()[m_atomId] = m_newPosition3d;
  }

  void undo() AVO_OVERRIDE
  {
    positions3d()[m_atomId] = m_oldPosition3d;
  }
};
} // end anon namespace
...
RWMolecule::BondType RWMolecule::addBond(Index atom1, Index atom2,
                                         unsigned char order, Index bondUid)
{
  if (atom1 == atom2
      || std::max(atom1, atom2) >= atomCount()
      || bondUid >= m_bondUniqueIds.size()
      || m_bondUniqueIds[bondUid] != MaxIndex) {
    return BondType();
  }
...
namespace {
class SetBondOrderCommand : public RWMolecule::UndoCommand
{
  Index m_bondId;
