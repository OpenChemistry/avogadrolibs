                                      ...
                                      const std::vector<unsigned short> &array)
{
  ...
  if (location == -1)
    return false;
  const GLvoid *data = reinterpret_cast<const GLvoid *>(&array[0]);
                                      ...
                                      const std::vector<Vector2f> &array)
{
  ...
  if (location == -1)
    return false;
  const GLvoid *data = reinterpret_cast<const GLvoid *>(array[0].data());
...
inline int ShaderProgram::findAttributeArray(const std::string &name)
{
  if (name.empty() || !m_linked)
    return -1;
  const GLchar *namePtr = reinterpret_cast<const GLchar *>(name.c_str());
  GLint location =
