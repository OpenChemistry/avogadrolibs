...
{
  public:
    AmbientOcclusionBaker(AmbientOcclusionRenderer *renderer, GLint textureSize)
      : m_renderer(renderer), m_textureSize(textureSize),
    ...
    Eigen::Matrix4f lookAt(const Vector3f &eye, const Vector3f &center,
                    Vector3f up)
    {
    ...
    void accumulateAO(int numDirections, const Vector3f &center, float radius)
    {
      assert(numDirections <= 500); // colors are unsigned bytes...
      ...
      Eigen::Matrix4f projection = ortho(-radius, radius, -radius, radius, -radius, radius);
      ...
      for (int i = 0; i < numDirections; ++i) {
        ...
        Vector3f dir = randomNormal();
        Eigen::Matrix4f modelView = lookAt(dir, Vector3f(0, 0, 0), Vector3f(0, 1, 0));

        // render depth to texture
        renderDepth(modelView, projection);
        // accumulate AO
        renderAO(modelView, projection, numDirections);
      }
  ...
  private:
    float randomFloat(float low = -1.0f, float high = 1.0f) const
    {
      // [0, RAND_MAX]
      float n = rand();
      // [0, 1]
    ...
    Vector3f randomNormal() const
    {
      while (true) {
        Vector3f n(randomFloat(), randomFloat(), randomFloat());
    ...
    struct OpenGLState
    {
      void save()
      {
        ...
        // polygon offset
        GLboolean polygonOffset = glIsEnabled(GL_POLYGON_OFFSET_FILL);
  ...
  public:
    ...
    void renderDepth(const Eigen::Matrix4f &modelView, const Eigen::Matrix4f &projection)
    {
      ...
      if (!m_depthProgram.setUniformValue("u_modelView",
                                          Eigen::Matrix3f(modelView.block<3, 3>(0, 0)))) {

        cout << m_depthProgram.error() << endl;
      }
      if (!m_depthProgram.setUniformValue("u_projection",
                                          Eigen::Matrix3f(projection.block<3, 3>(0, 0)))) {
...
void AmbientOcclusionSphereGeometry::update()
{
  ...
  if (!d->vbo.ready() || m_dirty) {
    ...
    for (unsigned int i = 0;
      ...
      ColorTextureVertex vert(itSphere->center - center, itSphere->color,
                              Vector2f(-r, -r),
    ...
    baker.accumulateAO(100, center, radius + 2.0);
...
void AmbientOcclusionSphereGeometry::render(const Camera &camera)
{
  ...
  if (!d->program.setUniformValue("u_modelView",
                                  Eigen::Matrix4f(camera.modelView().matrix() * d->translate))) {
    cout << d->program.error() << endl;
  }
  if (!d->program.setUniformValue("u_invModelView",
                                  Eigen::Matrix3f((camera.modelView().matrix() * d->translate).block<3, 3>(0, 0).inverse()))) {
    cout << d->program.error() << endl;
  }
