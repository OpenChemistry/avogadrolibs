...
void GLRenderer::render()
{
  ...
  if (!m_arrayBuffer.ready()) {
    m_arrayBuffer.upload(m_scene.m_spheres);
    m_indexBuffer.upload(m_scene.m_sphereIndices);
  }

  if (m_vertex.type() == Shader::Unknown) {

    // Let's try some bill boarding out...
  ...
  glDrawRangeElements(GL_TRIANGLES, 0, m_scene.m_sphereIndices.size(),
                      m_scene.m_sphereIndices.size(), GL_UNSIGNED_INT,
