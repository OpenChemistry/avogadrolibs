...
public:
  struct PackedVertex
  {
    ...
    PackedVertex()
      : offset(0, 0), tcoord(0.f, 0.f) {}

    static int offsetOffset() { return 0; }
    static int tcoordOffset()
    {
      return static_cast<int>(sizeof(Vector2i));
    }
  };
    ...
    const Array<unsigned char> &buffer, const Vector2i &dimensions)
{
  ...
  // Upload texture
  texture.upload(buffer, dimensions,
                 Texture2D::IncomingRGBA, Texture2D::InternalRGBA);
...
void TextLabelBase::RenderImpl::render(const Camera &cam)
{
  ...
  // Bind vbo
  if (!vbo.bind()) {
    std::cerr << "Error while binding TextLabelBase VBO: "
              << vbo.error() << std::endl;
    return;
  }
...
void TextLabelBase::buildTexture(const TextRenderStrategy &tren)
{
  ...
  const Vector2i newDims(bbox[1] - bbox[0] + 1,
                         bbox[3] - bbox[2] + 1);
