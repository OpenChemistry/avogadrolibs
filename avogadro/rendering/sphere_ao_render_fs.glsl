...
void main()
{
  ...
  vec3 diffuse = 0.5 * v_color * cos_alpha;
  ...
  vec3 specular = 0.4 * vec3(0.5, 1, 1) * pow(cos_beta, 50.0);

  // final color
  vec3 color = ambient + diffuse + specular;
  gl_FragColor = 1.5 * vec4(color, 1.0) * texture2D(u_tex, uv); // AO + Phong reflection [+ contours]
  //gl_FragColor = vec4(color, 1.0); // Phong reflection [+ contours]
  //gl_FragColor = 1.5 * texture2D(u_tex, uv); // AO [+ contours]
