...
TEST(CoordinateBlockGeneratorTest, generateCoordinateBlock)
{
  ...
  for (unsigned char i = 0; i < Elements::elementCount(); ++i) {
    double iPlusOne = static_cast<double>(i + 1);
    pos.x() = std::fmod(pos.x() * 12.3456789 * iPlusOne, 20.0);
    pos.y() = std::fmod(pos.y() * 98.7654321 * iPlusOne, 20.0);
    pos.z() = std::fmod(pos.z() * 56.7891234 * iPlusOne, 20.0);
