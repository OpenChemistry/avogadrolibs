...
TEST(CoordinateBlockGeneratorTest, generateCoordinateBlock)
{
  ...
  for (unsigned char i = 0; i < Elements::elementCount(); ++i) {
    float iPlusOne = static_cast<float>(i + 1);
    pos.x() = std::fmod(pos.x() * 12.3456789f * iPlusOne, 20.f);
    pos.y() = std::fmod(pos.y() * 98.7654321f * iPlusOne, 20.f);
    pos.z() = std::fmod(pos.z() * 56.7891234f * iPlusOne, 20.f);
    molecule.addAtom(i).setPosition3d(pos);
  }
