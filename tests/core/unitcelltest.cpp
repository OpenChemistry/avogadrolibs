...
#define RTSO_DO_TEST \
  ...
  EXPECT_FLOAT_EQ(a, mol.unitCell()->a()); \
  EXPECT_FLOAT_EQ(b, mol.unitCell()->b()); \
  EXPECT_FLOAT_EQ(c, mol.unitCell()->c()); \
  EXPECT_FLOAT_EQ(alpha, mol.unitCell()->alpha()); \
  EXPECT_FLOAT_EQ(beta,  mol.unitCell()->beta()); \
  EXPECT_FLOAT_EQ(gamma, mol.unitCell()->gamma())
...
TEST(UnitCellTest, setVolume)
{
  ...
  EXPECT_FLOAT_EQ(static_cast<Real>(1.0), mol.unitCell()->volume());
  EXPECT_FLOAT_EQ(static_cast<Real>(90), mol.unitCell()->alpha() * RAD_TO_DEG);
  EXPECT_FLOAT_EQ(static_cast<Real>(120), mol.unitCell()->beta() * RAD_TO_DEG);
  EXPECT_FLOAT_EQ(static_cast<Real>(77), mol.unitCell()->gamma() * RAD_TO_DEG);
}
...
TEST(UnitCellTest, fractionalCoordinates)
{
  ...
  for (int i = 0; i < 6; ++i) {
    for (int j = 0; j < 3; ++j) {
      EXPECT_FLOAT_EQ(ccoords_ref[i][j], ccoords[i][j])
          << " (i=" << i << "j=" << j << ")";
    }
  }
}
